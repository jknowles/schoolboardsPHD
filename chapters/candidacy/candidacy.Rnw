...
of public education and setting property tax limits.\footnote{Though in practice 
...
The strategic view of candidate entry argues that candidates emerge for 
...
and increases the vote-share of such candidates \citep{JacobsonStrategy}. Such 
strategic timing is foundational finding that matches well with the evidence that 
...
for office. For one, it has little to say about why a candidate 
...
politically marginalized group \citep{AJPS:AJPS147}. It may also be seen as a 
status symbol, an act of service, or a curiousity \citep{CanonArt, CanonBook}. 
By explaining the initial choice to run for office these authors argue we can 
paint a richer picture of the supply of candidates and the pool from which 
candidates for high office will be drawn. 
...
after the emergence of the rational model. A more quantiative return to 
the study of such political ambition emrged with \citet{AJPS:AJPS147}'s study of 
political psychology directed at explaining the ``nascent'' aspects of political ambition
through predictive features of candidates themselves. Today, such studies attempt 
to explain what the rational model does not -- entry into first office. 
...
professors are surprisingly absent from the ballot. This requires further 
...
of the rational model by explaining how the pool of potential candidates is 
...
such a way as to foster the development of quality candidates necessary for democratic legitimacy. 
...
school board members \citep{RefWorks:321, RefWorks:322, RefWorks:333}. However, 
...
in and out of the sample.\footnote{For now the standard deviations are influenced by 
the state's largest school district, Milwaukee, not being included in the sample while 
the election records are still processed.} The sample and the full data look remarkable 
close on all measures. Each year a different set of schools is in the sample owing 
the variable availability of records from school districts, but the sample size 
is consistent. 

% Graph showing number of districts with data for each year and number of years per district
...
Strategic Candidates  & \citet{Plutzer2002} & Change in partisanship preceeds increase in candidates \\ \hline
...
district measured by recertification strength, and more precise measures of 
...
<<modelsplot, fig.cap="Comparing Coefficients for the 4 Models of School Board Election Contestation", fig.height=9>>=
