...
selectVars <- c("TOTPOP.x", "toprepshare.x", "member", "econ_disadv_per", 
...
print(xtable(ab[-1,], label="tab:samplecomp", caption = "Comparing school districts that provided board election results to those that did not and are not included in the study." ), format.args = list(big.mark=","), include.rownames=TRUE, size="tiny")
...
p1 <- qplot(factor(Var1), Freq, data=a[a$Var1 > 2002 & a$Var1 < 2013,], geom='bar', stat='identity', fill=Var2, position='stack') + 
...
p2 <- qplot(factor(Var1), Freq, data=a[a$Var1 > 2002 & a$Var1 < 2013,], geom='bar', stat='identity', fill=Var2, position='stack')+ 
...
SBELEC$politicaldivide <- abs(SBELEC$toprepshare.x - .5)
...
poli_model <- glm(contested2 ~ primary2 + cum_attptsD + perwhite + Per65over + 
                   attemptsD + log(politicaldivide) + PerBachelorOrAbove +
                    median_income + log(Population18O), 
                 data=SBELEC, family=binomial("logit"))
...
<<candidatedescriptives3>>=

SBELEC$secturnout <- SBELEC$sectot.x / SBELEC$VAP_adj.x
SBELEC$recallpartisan <- abs(.5 - SBELEC$secrepshare.x)
library(arm)
simple_model <- bayesglm(contested2 ~  millrate_delta + 
                      recallpartisan + secturnout +
                   balance_member_lag + member_delta + locale3, 
                 data=SBELEC[SBELEC$year == 2011, ], family="binomial")
