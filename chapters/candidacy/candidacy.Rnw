...
Demographic variables (from Oliver): 
  - Percent white (per_white_all)
  - Median income (median_income)
  - Age (per65o)
  - Size (pop) (VAP_adjLOG)
  - OOH share (OOH_share)
  - Public Pupils (dist_turn$TOT_COUNT - dist_turn$TOTAL_COUNT_PVT)
  - Education level (PerBachelorOrAbove)

Fiscal variables (scope): 
  - Property tax revenue share (PROPERTYTAX_REV_SHARE)
  - Balance (balance_member)
  - Per pupil expenditure (TCEC_MEMBER, TDC_MEMBER, TEC_MEMBER)
...
Policy variables ()
  ...
  - Math performance (MATH_PROFADV_PER)
  - Reading performance (READ_PROFADV_PER)
  ...
  - Admin share of salary (ADMIN_SHARE)

School district data that helps sort this out. 

<<demog1, fig.cap="Correlations of Demographic Variables", fig.height=5, fig.width=5.5>>=
# library(GGally)
...
demogIdx <- c("per_white_all", "median_income", "per65o", "VAP_adjLOG", 
...
fiscalIdx <- c("PROPERTYTAX_REV_SHARE", "balance_member_lag", "TCEC_MEMBER", "TDC_MEMBER", 
               "NonSDMill", "millrate", "schoolMillShare")
...
# 
# ggpairs(dist_turn[dist_turn$year == 2011 & !is.na(dist_turn$turnout),], 
#         columns = which(names(dist_turn) %in% demogIdx), 
#         lower = list(continuous = "smooth"), 
#         upper = list(continuous = 'cor'), 
#         title = "Pairs Plot of Key Control Variables")
# 
# ggpairs(dist_turn[dist_turn$year == 2011 & !is.na(dist_turn$turnout),], 
#         columns = which(names(dist_turn) %in% policyIdx), 
#         lower = list(continuous = "smooth"), 
#         upper = list(continuous = 'cor'), 
#         title = "Pairs Plot of Key Control Variables")
# 
# ggpairs(dist_turn[dist_turn$year == 2011 & !is.na(dist_turn$turnout),], 
#         columns = which(names(dist_turn) %in% politicIdx), 
#         lower = list(continuous = "smooth"), 
#         upper = list(continuous = 'cor'), 
#         title = "Pairs Plot of Key Control Variables")
...
cormat <- melt(round(cor(dist_turn[!is.na(dist_turn$turnout), 
              which(names(dist_turn) %in% demogIdx)]), 3)) 
...
<<fiscal1, fig.cap="Correlations of Fiscal Variables", fig.height=5, fig.width=5.5>>=
...
<<politic1, fig.cap="Correlations of Fiscal Variables", fig.height=5, fig.width=5.5>>=
cormat <- melt(round(cor(dist_turn[!is.na(dist_turn$turnout), 
...
<<policy1, fig.cap="Correlations of Fiscal Variables", fig.height=5, fig.width=5.5>>=
...
xtable(round(cor(dist_turn[!is.na(dist_turn$turnout), 
              which(names(dist_turn) %in% policyIdx)], use = "pairwise"), 3))

xtable(round(cor(dist_turn[!is.na(dist_turn$turnout), 
              which(names(dist_turn) %in% fiscalIdx)], use = "pairwise"), 3))
...
moddat <- dist_turn[, c("contestSer", "contestMinor", "distid","year", 
                        ...
                        "OVERRIDE_ATTEMPT_CUMUL93", "ADJ_MEDIAN_SALARY_TEACHlog",
                        "teachSalDiff", "NEW_SUP", 
                        "turnoutLag2", "partyDivision", "balance_member_lag",
...
# }
# 
# for(i in unique(moddat$distid)){
#   moddat[moddat$distid == i, nums] <- 
#     apply(moddat[moddat$distid == i, nums], 2, rescale)
# }
...
noDistEff <- glm(contestSer ~ VAP_adjLOG + PerBachelorOrAbove + per_white_all + 
                    OOH_share +  per65o + median_incomeLOG + nraces + 
                    NonWhitePublicPupilPer + NonPublicPupilPer + 
                    ADMIN_SHARE_COMP +   
                    districtwide + econ_disadv_per+ 
                     tax_price + eqv_taxfiler + 
                     contestSerLag + incumShare + fallTurnout + 
                   DEBT_ATTEMPT_CUMUL93 +  
                   teachSalDiff + partyDivision + fallTwoPartyShareDem + 
                    balance_member_lag + 
                     NEW_SUP + overridePass + overrideQues , 
                 data = moddat, family = "binomial")


biggerMod <- glmer(contestSer ~ VAP_adjLOG + PerBachelorOrAbove + per_white_all + 
                    OOH_share +  per65o + median_incomeLOG + nraces +
                    NonWhitePublicPupilPer + NonPublicPupilPer + 
                    ADMIN_SHARE_COMP +   
                    districtwide + econ_disadv_per+ 
...
biggerModM <- glmer(contestMinor ~ VAP_adjLOG + 
                      PerBachelorOrAbove + per_white_all + 
                    OOH_share +  per65o + median_incomeLOG + nraces+
                    NonWhitePublicPupilPer + NonPublicPupilPer + 
                    ADMIN_SHARE_COMP +   
                    districtwide + econ_disadv_per+ 
                   ...
                   teachSalDiff + partyDivision + fallTwoPartyShareDem + 
                    balance_member_lag + 
                     NEW_SUP + overrideYesPer + overrideQues  + 
                     (1|distid), data = moddat, family = "binomial", 
                   control = ctrl)

biggerMod2 <- update(biggerMod, . ~ . - incumShare)
...
apsrtable(noDistEff, biggerMod, biggerMod2, biggerModM, se = "vcov", 
          float = "longtable",
          label="tab:bigmods", 
          model.names=c("Serious Cand. Lag", "Serious Cand. ME", 
                        "Serious Cand. ME2",
                        "Minor Cand. ME"), 
...
<<demogModel, results='asis'>>=
moddat <- dist_turn[, c("contestSer", "contestMinor", "distid","year", 
                        "balance_member_lag", "incumRun", "incumShare",
                        "PerBachelorOrAbove", "VAP_adjLOG", "per_white_all", 
                        "districtwide",
                        "OOH_share", "median_incomeLOG", "per65o", 
                        "NonWhitePublicPupilPer", "NonPublicPupilPer", 
...
# Rescale
library(arm)
moddat$NonWhitePublicPupilPer <- rescale(moddat$NonWhitePublicPupilPer)
...
moddat$NonPublicPupilPer <- rescale(moddat$NonPublicPupilPer)
moddat$teachShareofVoters <- rescale(moddat$teachShareofVoters)
moddat$member_delta_per <- rescale(moddat$member_delta_per)

demogModel <- glm(contestSer ~ VAP_adjLOG + PerBachelorOrAbove + per_white_all + 
                    OOH_share + median_incomeLOG + per65o + 
                    NonWhitePublicPupilPer + NonPublicPupilPer + 
                    teachShareofVoters + locale2 + member_delta_per + 
                    districtwide, 
                  data = moddat, family = "binomial")

# confusion.glm(demogModel$model, demogModel)

demogModel2 <- glm(contestSer ~ VAP_adjLOG + PerBachelorOrAbove + per_white_all + 
                    OOH_share + median_incomeLOG + per65o + 
                    NonWhitePublicPupilPer + NonPublicPupilPer + 
                    teachShareofVoters + locale2 + member_delta_per + 
                    districtwide + factor(year), 
                  data = moddat, family = "binomial")

demogModel3 <- glm(contestSer ~ VAP_adjLOG + PerBachelorOrAbove + per_white_all + 
                    OOH_share + median_incomeLOG + per65o + 
                    NonWhitePublicPupilPer + NonPublicPupilPer + 
                    teachShareofVoters + locale2 + member_delta_per + 
                     districtwide + distid, 
...
demogModel4 <- glm(contestMinor ~ VAP_adjLOG + PerBachelorOrAbove + per_white_all + 
                    OOH_share + median_incomeLOG + per65o + 
                    NonWhitePublicPupilPer + NonPublicPupilPer + 
                    teachShareofVoters + locale2 + member_delta_per + 
                     districtwide + distid, 
...
apsrtable(demogModel2, demogModel3, demogModel4, se = "vcov", 
          float = "sidewaystable",
          label="tab:demogmods", 
          model.names=c("Simple Demog.", "Demog Non Minor Cand", 
                        "Demog Minor Cand"), 
          caption = "Models of Contestation in School Board Elections",
          notes = list(stars.note, 
...
<<partialPooldemog, results='asis'>>=
demogModelPP1 <- glmer(contestSer ~ VAP_adjLOG + PerBachelorOrAbove + per_white_all + 
                    OOH_share + median_incomeLOG + per65o + 
                    NonWhitePublicPupilPer + NonPublicPupilPer + 
                    teachShareofVoters + locale2 + member_delta_per + 
                     (1|distid), 
...
demogModelPP2 <- glmer(contestMinor ~ VAP_adjLOG + PerBachelorOrAbove + per_white_all + 
                    OOH_share + median_incomeLOG + per65o + 
                    NonWhitePublicPupilPer + NonPublicPupilPer + 
                    teachShareofVoters + locale2 + member_delta_per + 
                     (1|distid), 
...
# 
# demogModelPP <- bglmer(contestSer ~ VAP_adjLOG + PerBachelorOrAbove + per_white_all + 
#                     OOH_share + median_incomeLOG + per65o + 
#                     NonWhitePublicPupilPer + NonPublicPupilPer + 
#                     teachShareofVoters + locale2 + member_delta_per + 
#                      (1|distid), 
...
# demogModelPP2 <- bglmer(contestSer ~ VAP_adjLOG + PerBachelorOrAbove + 
#                           per_white_all + 
#                     OOH_share + median_incomeLOG + per65o + 
#                     NonWhitePublicPupilPer + NonPublicPupilPer + 
#                     teachShareofVoters + locale2 + member_delta_per + 
...
#                   cov.prior = wishart)
# 
# demogModelPP3 <- bglmer(contestSer ~ VAP_adjLOG + PerBachelorOrAbove + 
#                           per_white_all + 
#                     OOH_share + median_incomeLOG + per65o + 
#                     NonWhitePublicPupilPer + NonPublicPupilPer + 
#                     teachShareofVoters + locale2 + member_delta_per + 
...
#                   cov.prior = wishart)
# 
# class(demogModelPP) <- "merMod"
# class(demogModelPP2) <- "merMod"
# class(demogModelPP3) <- "merMod"


apsrtable(demogModelPP1, demogModelPP2,
          model.names = c("No Minor Candidates", 
                          "Minor Candidates Included"),
          caption = "Partial Pooling Model of Demographics on Turnout",
          label = "tab:meDemog", Sweave=FALSE, float = "sidewaystable", 
          notes = list("Group level intercepts are not shown.", 
                       stars.note, rescalenote), 
          digits = 3)
...
<<fiscalMod, results='asis'>>=
moddat <- dist_turn[, c("contestSer", "contestMinor", "distid","year", 
                        "balance_member_lag",
                        "millrate", "VAP_adjLOG", "NonSDMill", 
                        "districtwide", "OOH_share",
                        "overridePass", "median_incomeLOG", "debtPass", 
                        "millrate_delta", "refInPlace", 
                        "DEBT_ATTEMPT_CUMUL93", "eqv_member", 
                        "OVERRIDE_ATTEMPT_CUMUL93", "ADJ_MEDIAN_SALARY_TEACHlog",
                        "member_delta_per", "teachSalDiff")]
...
# moddat <- na.omit(moddat)
# Rescale
moddat$balance_member_lag <- rescale(moddat$balance_member_lag)
moddat$millrate <- rescale(moddat$millrate)
moddat$NonSDMill <- rescale(moddat$NonSDMill)
moddat$median_incomeLOG <- rescale(moddat$median_incomeLOG)
moddat$millrate_delta <- rescale(moddat$millrate_delta)
moddat$VAP_adjLOG <- rescale(moddat$VAP_adjLOG)
moddat$eqv_member <- rescale(moddat$eqv_member)
moddat$teachSalDiff <- rescale(moddat$teachSalDiff)

fiscalModel <- glm(contestSer ~ balance_member_lag + millrate * NonSDMill + 
                     overridePass + debtPass + refInPlace + median_incomeLOG + 
                     millrate_delta + VAP_adjLOG + 
                     DEBT_ATTEMPT_CUMUL93 + eqv_member + teachSalDiff +
                     OVERRIDE_ATTEMPT_CUMUL93, 
                   data = moddat, family = 'binomial')

fiscalModel2 <- glm(contestSer ~ balance_member_lag + millrate * NonSDMill + 
                     overridePass + debtPass + refInPlace + median_incomeLOG + 
                     millrate_delta + VAP_adjLOG + 
                     DEBT_ATTEMPT_CUMUL93 + eqv_member + teachSalDiff +
                     OVERRIDE_ATTEMPT_CUMUL93 + year, 
                   data = moddat, family = 'binomial')
                   
fiscalModel3 <- glm(contestSer ~ balance_member_lag + millrate * NonSDMill + 
                     overridePass + debtPass + refInPlace + median_incomeLOG + 
                     millrate_delta + VAP_adjLOG + 
                     DEBT_ATTEMPT_CUMUL93 + eqv_member + teachSalDiff +
                     OVERRIDE_ATTEMPT_CUMUL93 + distid, 
                   data = moddat, family = 'binomial')

apsrtable(fiscalModel, fiscalModel2, fiscalModel3,
          label="tab:fiscalmods", 
          model.names=c("Simple Fiscal", "Fiscal + Year FE", 
                        "Fiscal + District FE"), 
...
<<fiscalModPP, results='asis'>>=
ctrl <- glmerControl(optimizer = "optimx", optCtrl = list(method = "nlminb", 
                                                          maxit = 4000))


fiscalModelPP <- glmer(contestSer ~ balance_member_lag + millrate * NonSDMill + 
                     overridePass + debtPass + refInPlace + median_incomeLOG + 
                     millrate_delta + VAP_adjLOG + 
                     DEBT_ATTEMPT_CUMUL93 + eqv_member + teachSalDiff +
                     OVERRIDE_ATTEMPT_CUMUL93 + (1|year), 
                   data = moddat, family = 'binomial')

fiscalModelPP2 <- glmer(contestSer ~ balance_member_lag + millrate * NonSDMill + 
                     overridePass + debtPass + refInPlace + median_incomeLOG + 
                     millrate_delta + VAP_adjLOG + 
                     DEBT_ATTEMPT_CUMUL93 + eqv_member + teachSalDiff +
                     OVERRIDE_ATTEMPT_CUMUL93 + (1|distid), 
                   data = moddat, family = 'binomial', control = ctrl)

fiscalModelPP3 <- glmer(contestMinor ~ balance_member_lag + millrate *millrate_delta +  NonSDMill + 
                     overridePass + debtPass + refInPlace + median_incomeLOG + 
                     millrate_delta + VAP_adjLOG + 
                     DEBT_ATTEMPT_CUMUL93 + eqv_member + teachSalDiff +
                     OVERRIDE_ATTEMPT_CUMUL93 + (1|distid), 
...
# class(fiscalModelPP) <- "merMod"
# class(fiscalModelPP2) <- "merMod"
# class(fiscalModelPP3) <- "merMod"
 
apsrtable(fiscalModelPP, fiscalModelPP2, fiscalModelPP3,
          model.names = c("Fiscal + Year Eff.", 
                          "Fiscal + District Eff.", 
                          "Fiscal NonMinor Dist Eff."),
          caption = "Partial Pooling Model of Fiscal on Turnout",
         label = "tab:meFiscal", Sweave=FALSE, float = "sidewaystable", 
          notes = list("Group level intercepts are not shown.", 
                       stars.note, rescalenote), digits = 3 )
@
...
<<politicMod, results = 'asis'>>=
moddat <- dist_turn[, c("contestSer", "NEW_SUP", "distid","year", 
                        "turnoutLag1",  "contestSerLag2", "contestMinorLag2",
                        "partyDivision", "VAP_adjLOG", "overrideQues", 
                        "districtwide", "debtQues", "overrideYesPer",
                        "overridePass", "median_incomeLOG", "debtPass", 
                        "fallTurnout", "SUP_TURNOVER_PRIOR_YEAR", 
                        "DEBT_ATTEMPT_CUMUL93", "contestMinor", 
                        "OVERRIDE_ATTEMPT_CUMUL93", "ADJ_MEDIAN_SALARY_TEACHlog",
                        "teachSalDiff")]
...
moddat$partyDivision <- rescale(moddat$partyDivision)
moddat$median_incomeLOG <- rescale(moddat$median_incomeLOG)                        
moddat$fallTurnout <- rescale(moddat$fallTurnout)                        
moddat$ADJ_MEDIAN_SALARY_TEACHlog <- rescale(moddat$ADJ_MEDIAN_SALARY_TEACHlog)  
moddat$OVERRIDE_ATTEMPT_CUMUL93 <- rescale(moddat$OVERRIDE_ATTEMPT_CUMUL93)
moddat$DEBT_ATTEMPT_CUMUL93 <- rescale(moddat$DEBT_ATTEMPT_CUMUL93)

politicModel <- glm(contestSer ~ SUP_TURNOVER_PRIOR_YEAR + turnoutLag1 + partyDivision + 
                      overrideQues +   debtQues + DEBT_ATTEMPT_CUMUL93 +  contestSerLag2 +
                       OVERRIDE_ATTEMPT_CUMUL93 + ADJ_MEDIAN_SALARY_TEACHlog *
                      median_incomeLOG + fallTurnout + overrideYesPer, 
                    data = moddat, family = "binomial")

politicModel2 <- glm(contestSer ~ SUP_TURNOVER_PRIOR_YEAR + turnoutLag1 + partyDivision + 
                      overrideQues +   debtQues + DEBT_ATTEMPT_CUMUL93 + contestSerLag2 +
                       OVERRIDE_ATTEMPT_CUMUL93 + ADJ_MEDIAN_SALARY_TEACHlog *
                      median_incomeLOG + fallTurnout + overrideYesPer + 
                      year, 
...
politicModel3 <- glm(contestSer ~ SUP_TURNOVER_PRIOR_YEAR + turnoutLag1 + partyDivision + 
                      overrideQues +   debtQues + DEBT_ATTEMPT_CUMUL93 + contestSerLag2 +
                       OVERRIDE_ATTEMPT_CUMUL93 + ADJ_MEDIAN_SALARY_TEACHlog *
                      median_incomeLOG + fallTurnout + overrideYesPer + 
                      distid, 
...
politicModel4 <- glm(contestMinor ~ SUP_TURNOVER_PRIOR_YEAR + turnoutLag1 + partyDivision + 
                      overrideQues +   debtQues + DEBT_ATTEMPT_CUMUL93 + contestSerLag2+
                       OVERRIDE_ATTEMPT_CUMUL93 + ADJ_MEDIAN_SALARY_TEACHlog *
                      median_incomeLOG + fallTurnout + overrideYesPer + 
                      distid, 
...
nullModelFE <- glm(contestSer ~ 1 + distid, data = moddat, family = 'binomial')
nullModel <- glm(contestSer ~ 1, data = moddat, family = 'binomial')

apsrtable(politicModel, politicModel2, politicModel3,
          label="tab:politicmods", 
          model.names=c("Simple Politic", "Politic + Year FE", 
                        "Politic + District FE"), 
...
<<politicModPP, results = 'asis'>>=
# politicModelPP <- glmer(contestSer ~ SUP_TURNOVER_PRIOR_YEAR + turnoutLag1 + partyDivision + 
#                       overrideQues +   debtQues + DEBT_ATTEMPT_CUMUL93 + contestSerLag2 +
#                        OVERRIDE_ATTEMPT_CUMUL93 + ADJ_MEDIAN_SALARY_TEACHlog *
#                       median_incomeLOG + fallTurnout + overrideYesPer + (1|year), 
#                    data = moddat, family = 'binomial', control = ctrl)

politicModelPP2 <- glmer(contestSer ~ SUP_TURNOVER_PRIOR_YEAR + turnoutLag1 + partyDivision + 
                      overrideQues + debtQues + DEBT_ATTEMPT_CUMUL93 +  contestSerLag2 +
                       OVERRIDE_ATTEMPT_CUMUL93 + ADJ_MEDIAN_SALARY_TEACHlog *
                      median_incomeLOG + fallTurnout + overrideYesPer + (1|distid), 
                   data = moddat, family = 'binomial', control = ctrl)


politicModelPP3 <- glmer(contestMinor ~ SUP_TURNOVER_PRIOR_YEAR + turnoutLag1 + partyDivision + 
                      overrideQues +   debtQues + DEBT_ATTEMPT_CUMUL93 +  contestSerLag2 +
                       OVERRIDE_ATTEMPT_CUMUL93 + ADJ_MEDIAN_SALARY_TEACHlog *
                      median_incomeLOG + fallTurnout + overrideYesPer + (1|distid), 
                   data = moddat, family = 'binomial', control = ctrl)

nullModelPP <- glmer(contestSer ~ 1 + (1|distid), 
                   data = moddat, family = 'binomial', control = ctrl)

# politicModelPP3 <- bglmer(contestSer ~ NEW_SUP + turnoutLag1 + partyDivision + 
#                       overrideQues +  debtPass + debtQues + 
#                       SUP_TURNOVER_PRIOR_YEAR +
#                       median_incomeLOG + fallTurnout + overrideYesPer + (1|distid) + (1|year), 
#                    data = moddat, family = 'binomial', 
#                    cov.prior = wishart)
# 
# class(politicModelPP) <- "merMod"
# class(politicModelPP2) <- "merMod"
# class(politicModelPP3) <- "merMod"

apsrtable(politicModelPP2, politicModelPP3,
          model.names = c("Serious Contest", 
                          "Minor Contest"),
          caption = "Partial Pooling Model of Politics on Turnout",
          label = "tab:mePolitics", Sweave=FALSE, float = "sidewaystable", 
          notes = list("Group level intercepts are not shown.", rescalenote,
                       stars.note), 
          digits = 3)
...
roc <- makeROCdf(demogModelPP2, "demog")
roc2 <- makeROCdf(politicModelPP2, "politics")
roc3 <- makeROCdf(fiscalModelPP2, "fiscal")
roc4 <- makeROCdf(nullModel, "null")

roc <- makeROCdf(demogModel2, "demog")
roc2 <- makeROCdf(politicModel2, "politics")
roc3 <- makeROCdf(fiscalModel2, "fiscal")
roc4 <- makeROCdf(nullModel, "null")



roc <- rbind(roc, roc2); rm(roc2)
roc <- rbind(roc, roc3); rm(roc3)
roc <- rbind(roc, roc4); rm(roc4)
@

<<rocplot1, echo=FALSE, eval = FALSE, fig.cap="ROC Curves for 4 Models of Election Contestation in Wisconsin School Boards", fig.keep='last'>>=
ggplot(roc) + 
#   geom_smooth(aes(x = fpr, y = tpr, color=mod), 
#                           se=FALSE,size= I(1.1)) + 
  geom_line(aes(x = fpr, y = tpr, color=mod), 
                          size= I(1.1)) +
   geom_abline(intercept = 0, slope = 1, colour = "gray") + 
   ylab("Sensitivity") + 
   xlab("1 - Specificity") + 
  coord_cartesian(xlim=c(0, 1), ylim=c(0,1)) + theme_bw() + 
  labs(title = "Comparing Models of Candidate Contestation in School Board Elections", 
       color = "Model")
...
<<anovatable1, results='asis', eval=FALSE>>=
print(xtable(anova(tax_model, poly_model, poli_model, uber_model, test="LRT")), 
...
<<modelsplot, eval = FALSE, fig.cap="Comparing Coefficients for the 4 Models of School Board Election Contestation", fig.height=9>>=

plotcoefs = c("total_attempts", "avg_total_exp", "log(politicaldivide)", 
          "Per65over", "average_fringe", "avg_salary", "log(dv_r)", 
          "PerBachelorOrAbove", "toprepshare", "attemptsD", "Per65over", 
          "contested2.lag1", "primary2Primary", "median_income",
          "balance_member_lag", "log(Population180)", "levy_chg", 
          "perwhite", "overlevy_ind_lag", "cum_attptsD", "millrate")


multiplot(poli_model, tax_model, poly_model, uber_model, intercept=FALSE, 
          coefficients = plotcoefs, sort = c("magnitude"), 
          title = "Change in Probability of A Contested Election") + 
  coord_trans(x = "asinh") + 
  theme_dpi() + theme(legend.position = "bottom")
...
<<ReviewTab, results='asis', eval=FALSE>>=
# stargazer(mixedMod1b, mixedMod2b, mixedMod3b, mixedModCaus, style = "apsr", 
#           label = "tab:modReview")
@


<<candidatedescriptives3, eval=FALSE, echo=FALSE>>=

# Base model with simple treatment dummy
# Time model with linear time for Act 10
# Longitudinal within district using lmer
# add dummies for years

SBELEC$pres_primary <- 0
SBELEC$pres_primary <- ifelse(SBELEC$year == 2008, 1, 0)
SBELEC$pres_primary <- ifelse(SBELEC$year == 2012, 1, SBELEC$pres_primary)


trtmnt_model <- bayesglm(contested2 ~  millrate_delta + contested2Lag2 +
                      RecallPolarization*treatment + GovTurnout_recall +  
                       log(Population18O) + treatment, 
                 data=SBELEC[SBELEC$year >= 2010 & SBELEC$year < 2013, ], 
                 family="binomial")

simple_model <- bayesglm(contested2 ~  millrate + contested2.lag1 +
                      walkerdivide + recallturnout +  pres_primary +
                       log(Population18O) + time + I(time^2), 
                 data=SBELEC[SBELEC$year >= 2009 & SBELEC$year < 2013, ], 
                 family="binomial")

summary(simple_model)

library(lme4)
simple_lmer <- glmer(contested2 ~ treatment + pres_primary + 
                       millrate + log(Population18O) +
                       contested2.lag1 + 
                       (1 + sqrt(walkerdivide) + recallturnout | distid), 
                     data=SBELEC[SBELEC$year >=2008 & SBELEC$year <2013, ], 
                     family = "binomial")

summary(simple_lmer)


dotplot(ranef(simple_lmer, condVar=TRUE))
##### 
# LME4

# 
# blahmod <- glmer(contested2 ~ log(dv_r) + avg_salary + average_fringe + 
#                     log(Population18O) + walkerdivide +
#                     median_income + primary2 + perwhite + Per65over + 
#                    attemptsD +
#                     millrate + overlevy_ind_lag + fyear+ (1|distid), 
#                  data = SBELEC, family = binomial("logit"))
# 
# 
# 
# blahmod2 <- glmer(contested2 ~ log(dv_r) + avg_salary + average_fringe + 
#                     log(Population18O) + 
#                     median_income + primary2 + perwhite + Per65over + 
#                    attemptsD +
#                     millrate + overlevy_ind_lag + fyear + 
#                     (1 + walkerdivide|distid), 
#                  data = SBELEC, family = binomial)
# 
# dotplot(ranef(blahmod2, condVar=TRUE))
# 
# anova(blahmod, blahmod2)
...
<<modTestDemog, results='hide', eval=FALSE, cache=TRUE>>=
library(pbkrtest)
library(doParallel)
cl <- makePSOCKcluster(3)

modTestDemog1 <- PBmodcomp(demogModelPP3, demogModelPP, nsim = 200, 
          cl = cl)

modTestDemog2 <- PBmodcomp(demogModelPP3, demogModelPP2, nsim = 200, 
          cl = cl)
...
modTestFiscal1 <- PBmodcomp(fiscalModelPP3, fiscalModelPP, nsim = 200, 
          cl = cl)

modTestFiscal2 <- PBmodcomp(fiscalModelPP3, fiscalModelPP2, nsim = 200, 
          cl = cl)
...
modTestPolitic1 <- PBmodcomp(politicModelPP3, politicModelPP, nsim = 200, 
          cl = cl)

modTestPolitic2 <- PBmodcomp(politicModelPP3, politicModelPP2, nsim = 200, 
          cl = cl)
