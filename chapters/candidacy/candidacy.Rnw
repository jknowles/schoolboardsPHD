...
vital to understanding the candidate pool for higher offices. By moving down to the level of state politicians, \citet{MaestasEtAl} 
...
(CITE PERSONAL CORRESPONDENCE)} In order to conduct this study then, a records 
...
ab <- as.data.frame(cbind(a[,2], b[,2], a[,1], a[,2]))
names(ab) <- c("Sample (mean)", "Sample (SD)", "Out of Sample (mean)", "Out of Sample (SD)")

print(xtable(ab[-1,]), format.args = list(big.mark=","))
...
As shown in table X, there is little difference between schools in and out of the sample. 
...
In general, the years surrounding Act 10 do not appear more contested than the 
...
tax_model <- glm(contested2 ~ primary + millrate_delta + total_attempts + 
                   wins +
                   balance_member_lag2 + member_delta2 + locale3, 
