...
tax_model <- glm(contested2 ~  millrate + cum_attptsD + overlevy_ind_lag +
                   ...
                   total_attempts.lag1 + winshare.lag1 +
...
poli_model <- glm(contested2 ~ primary2 + contested2.lag1 + 
                    cum_attptsD + perwhite + Per65over + 
                   attemptsD + toprepshare + PerBachelorOrAbove +
...
uber_model <- glm(contested2 ~ log(dv_r) + avg_salary + average_fringe + 
                   ...
                   attemptsD + log(politicaldivide) + PerBachelorOrAbove +
...
ggplot(roc) + geom_line(aes(x = fpr, y = tpr, color=mod), size= I(1.1)) + 
...
mermod1 <- glmer(contested2 ~  log(dv_r) +   
    ...
    perwhite +  log(politicaldivide) + 
    millrate + overlevy_ind_lag +  
     fyear + (1 + walkerdivide|distidF), 
...
stargazer(tax_model, poli_model, poly_model, uber_model, 
          omit = "fyear", font.size="scriptsize")
@
