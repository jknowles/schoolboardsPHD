...
fiscalModelPP3 <- glmer(contest2 ~ balance_member_lag + millrate * NonSDMill + 
                     overridePass + debtPass + refInPlace + median_incomeLOG + 
                     millrate_delta + VAP_adjLOG + 
                     DEBT_ATTEMPT_CUMUL93 + eqv_member + teachSalDiff +
                     OVERRIDE_ATTEMPT_CUMUL93 + (1|distid) + (1|year), 
                   data = moddat, family = 'binomial', control = ctrl)
...
politicModel <- glm(contest2 ~ NEW_SUP + turnoutLag1 + partyDivision + 
...
politicModel2 <- glm(contest2 ~ NEW_SUP + turnoutLag1 + partyDivision + 
                      overrideQues +  debtPass + debtQues + 
                      SUP_TURNOVER_PRIOR_YEAR +
                      median_incomeLOG + fallTurnout + overrideYesPer + 
                      year, 
...
politicModel3 <- glm(contest2 ~ NEW_SUP + turnoutLag1 + partyDivision + 
                      overrideQues +  debtPass + debtQues + 
                      SUP_TURNOVER_PRIOR_YEAR +
                      median_incomeLOG + fallTurnout + overrideYesPer + 
                      distid, 
...
politicModelPP <- bglmer(contest2 ~ NEW_SUP + turnoutLag1 + partyDivision + 
                      overrideQues +  debtPass + debtQues + 
                      SUP_TURNOVER_PRIOR_YEAR +
...
politicModelPP2 <- bglmer(contest2 ~ NEW_SUP + turnoutLag1 + partyDivision + 
                      overrideQues +  debtPass + debtQues + 
                      SUP_TURNOVER_PRIOR_YEAR +
