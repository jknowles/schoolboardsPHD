...
mod1 <- glm(contest1 ~ VAP_adjLOG + NEW_SUP + turnoutLag1 +fallTwoPartyShareDem +
              PerBachelorOrAbove + OOH_share + median_incomeLOG + ADMIN_SHARE + 
              per65o + log(ADJ_MEDIAN_FRINGE_TEACH) +
              NonWhitePublicPupilPer + overrideQues + millrate*NonSDMill + 
              overridePass + debtPass + SUP_TURNOVER_PRIOR_YEAR, 
