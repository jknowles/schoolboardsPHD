...
<<modelcomparisonD>>=
moddat <- dist_turn[, c("turnout", "turnoutLag2", "turnoutLag1","year", "contest1",
                        "contest2", "balance_member_lag", 
...
mod1 <- glm(contest2 ~ VAP_adjLOG + NEW_SUP + turnoutLag1 +partyDivision +
              ...
              NonWhitePublicPupilPer *per_white_all+ overrideQues + millrate*NonSDMill + 
              overridePass + debtPass, 
            data = dist_turn, family = "binomial")
summary(mod1)

confusion.glm(mod1$model, mod1)
