...
poli_model <- glm(contested2 ~ primary2 + cum_attptsD + perwhite + Per65over + 
...
predicted <- predict(tax_model)

prob <- prediction(predicted, tax_model$model$contested2)
...
predicted <- predict(poli_model)
prob <- prediction(predicted, poli_model$model$contested2)
tprfpr <- performance(prob, "tpr", "fpr")
tpr <- unlist(slot(tprfpr, "y.values"))
fpr <- unlist(slot(tprfpr, "x.values"))
roc2 <- data.frame(tpr, fpr)
roc2$mod <- "politics"

roc <- rbind(roc, roc2); rm(roc2)

predicted <- predict(poly_model)
prob <- prediction(predicted, poly_model$model$contested2)
tprfpr <- performance(prob, "tpr", "fpr")
tpr <- unlist(slot(tprfpr, "y.values"))
fpr <- unlist(slot(tprfpr, "x.values"))
roc2 <- data.frame(tpr, fpr)
...
predicted <- predict(uber_model)
prob <- prediction(predicted, uber_model$model$contested2)
tprfpr <- performance(prob, "tpr", "fpr")
tpr <- unlist(slot(tprfpr, "y.values"))
fpr <- unlist(slot(tprfpr, "x.values"))
roc2 <- data.frame(tpr, fpr)
...
simple_model <- bayesglm(contested2 ~  millrate_delta + 
                      walkerdivide + recallturnout +
                   balance_member_lag + member_delta + log(Population18O) + 
                     fyear, 
                 data=SBELEC[SBELEC$year >= 2009, ], family="binomial")
