...
<<modelcomparisonD, fig.cap="Comparing Tax, Politics, and Policy Models of Candidate Emergence in School Board Elections Using ROC">>=
...
ggplot(roc) + geom_line(aes(x = fpr, y = tpr, color=mod)) + 
...
# library(lme4)
# 
# mermod1 <- glmer(contested2 ~ primary2 + millrate + cum_attptsD + overlevy_ind_lag +
#                     perwhite + levy_chg + log(politicaldivide) + 
#                    balance_member_lag + median_income + 
#                    (1|fyear) + (1|distid), 
