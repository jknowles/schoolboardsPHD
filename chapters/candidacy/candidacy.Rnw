...
positively related to contestation of school board elections. In addition to 
...
in the school district that is white. While Wisconsin is predominantly a white 
...
I expect to see %TODO: citation for plutzer
...
positively related to board contestation %TODO: find citations \citet{}. 
...
governments like school boards %TODO: cite berry 
...
to other local governments. As \citet{berry} finds stacking of local governments 
...
to emerge when multiple races exist. Next, I test for advantage of incumbency 
...
to be unrelated to challengers emerging for school board. Next, I test the 
...
As a final measure, I follow the work of citet{moe,anzia} and explore the influence of teachers' unions in elections. Teachers' unions represent the 
...
to emerge. A strong interest group presence on the board should suppress 
...
policy changes in the district. %TODO: Not perfect due to timing
...
decrease in candidate challenges to board members %TODO: cite \cite{wu,baratc}. 
...
district to raise taxes to cover the payments on a loan to build infrastructure 
such as a new or renovated school building. Override referenda are specific 
...
election to exceed that limit with additional property tax revenue. Attempting 
...
to have less board challenges. %TODO: why
...
<<demog1, fig.cap="Correlations of Demographic Variables", fig.height=5, fig.width=6.25>>=
...
qplot(x=Var1, y=Var2, data=cormat, fill=value2, geom="tile", color = I("gray40")) + 
  ...
  geom_text(aes(label = value)) + nopanel + 
  labs(x = "", y = "")
@

<<fiscal1, fig.cap="Correlations of Fiscal Variables", fig.height=5, fig.width=6.25>>=
...
<<politic1, fig.cap="Correlations of Political Variables", fig.height=5, fig.width=6.25>>=
cormat <- melt(round(cor(dist_turn[!is.na(dist_turn$turnout), 
...
<<policy1, fig.cap="Correlations of Policy Variables", fig.height=5, fig.width=6.25>>=
...
<<candidatedescriptives4, echo=FALSE, fig.cap="Districts with contested school board elections under two different measures of candidate contestation.">>=
library(scales)
...
<<modTestDemog, results='hide', eval=FALSE>>=
library(pbkrtest)
library(doParallel)
...
<<modTestFiscal, results='hide', eval=FALSE>>=
cl <- makePSOCKcluster(4)
...
<<modTestPolitic, results='hide', eval=FALSE>>=
cl <- makePSOCKcluster(4)
...
<<modTestPolicy, results='hide', eval=FALSE>>=
cl <- makePSOCKcluster(4)
...
<<modTestFull, results='hide', eval=FALSE>>=
...
<<pbtestresults, results='asis', eval=FALSE>>=
...
printPB(modTestDemog1, names = nams1)
printPB(modTestDemog2, names = nams2)

printPB(modTestFiscal1, names = nams1)
printPB(modTestFiscal2, names = nams2)

printPB(modTestPolitic1, names = nams1)
printPB(modTestPolitic2, names = nams2)

printPB(modTestPolicy1, names = nams1)
printPB(modTestPolicy2, names = nams2)



@

<<pbresults, results='markup', eval=FALSE>>=
modTestDemog1
modTestDemog2
modTestFiscal1
modTestFiscal2
