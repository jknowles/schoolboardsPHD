...
at the end of the collection period, \Sexpr{print(SAMPSIZE)} districts responded 
with records. 
...
selectVars <- c("TOTPOP", "toprepshare", "member", "econ_disadv_per", 
                "millrate", "PerBachelorOrAbove", "avg_salary", "average_fringe", 
                "median_income", "OOH_share", "balance_lag")

rnames <- c("Total Population", "Republican Share", "Students", 
            "% FRL", "Millrate", "Share Bachelor +", "Average Salary", 
...
a <- as.data.frame(a); names(a) <- c("Out of Sample (Mean)", "Sample (Mean)")
b <- as.data.frame(b); names(b) <- c("Out of Sample (SD)", "Sample (SD)")
...
#aggregate(SBELEC$candidates, by=list(SBELEC$year), function(x) sum(x) / length(x))

# a <- table(SBELEC$contested, SBELEC$year)
# rownames(a) <- c("Uncontested", "Contested")
# print(xtable(a[, 3:13]), include.rownames=TRUE)

# Now toss out minor candidates

# 
# rownames(a) <- c("Uncontested", "Contested")
# print(xtable(a[, 3:13]), include.rownames=TRUE)

a <- table(SBELEC$contested2, SBELEC$year)
a <- t(a)
a <- as.data.frame(a)
...
p2 <- qplot(factor(Var1), Freq, data=a[a$Var1 > 2002 & a$Var1 < 2013,], geom='bar', stat='identity', fill=Var2, position='stack', color = I("black")) + 
  labs(x= "Year", y = "Districts", title = "Including Minor Candidates", 
       fill = "Contested")+ theme_bw()
...
qplot(fyear, contested3, data=SBELEC[!is.na(SBELEC$fyear),], geom='boxplot') + 
  theme_bw() + 
...
<<causaldataclean, echo=FALSE, results='hide'>>=
SBELEC$treatment <- 0
...
tax_model <- glm(contested2 ~  millrate + cum_attptsD + overlevy_ind_lag +
                    perwhite + levy_chg + log(Population18O) + 
                   ref_share +
...
poli_model <- glm(contested2 ~ contested2.lag1 + primary2.lag1 +
                    cum_attptsD + perwhite + Per65over + log_politicaldivide +
                   winshare.lag1 + attemptsD.lag1 +  toprepshare +
                    PerBachelorOrAbove +
...
poly_model <- glm(contested2 ~ log(dv_r) + avg_salary + average_fringe + 
...
<<rocplot1, echo=FALSE, fig.cap="ROC Curves for 4 Models of Election Contestation in Wisconsin School Boards", fig.keep='last'>>=
...
<<modelsplot, fig.cap="Comparing Coefficients for the 4 Models of School Board Election Contestation">>=
...
plotted for the four models.\footnote{Graph made using the excellent \texttt{coefplot} package for R along with \texttt{theme_dpi} theme for \texttt{ggplot2} provided 
...
<<descrmixedeff, eval=FALSE>>=

# Alternative is to model number of cnadidates as poisson
# 
# SBELEC$candidatesDV <- SBELEC$candidates - SBELEC$minorcand - SBELEC$winners
# SBELEC$candidatesDV <- ifelse(SBELEC$candidatesDV > -1, SBELEC$candidatesDV, 0)
# # Investigate why negative numbers come up here

library(lme4)

SBELEC$distidF <- factor(SBELEC$distid)
mermod1 <- glmer(contested2 ~  log(dv_r) +   
    avg_total_exp + log(Population18O) +  median_income + 
      cum_attptsD + attemptsD +
    perwhite +  log_politicaldivide + 
    millrate + overlevy_ind_lag +  
     fyear + (1 + log_politicaldivide|distidF), 
                 data=SBELEC[SBELEC$year > 2004, ], family="binomial")

nullmermod <- glmer(contested2 ~  log(dv_r) +   
    avg_total_exp + log(Population18O) +  median_income + 
      cum_attptsD + attemptsD +
    perwhite +  log_politicaldivide + 
    millrate + overlevy_ind_lag +  
...
# summary(mermod1)
# rr1 <- ranef(mermod1, condVar=TRUE)
# 
# confint(mermod1, method="Wald")
#dotplot(ranef(mermod1, condVar=TRUE))
@
...
trtmnt_model <- bayesglm(contested2 ~  millrate + contested2.lag1 +
                      walkerdivide + recallturnout +  pres_primary + 
                       log(Population18O) + treatment + distidF, 
...
stargazer(tax_model, poli_model, poly_model, uber_model, 
          omit = "fyear", font.size="scriptsize", label = "tab:descrmodels", 
          title = "Comparing 4 Models of Contestation in School Board Elections", 
          column.labels = c("Taxation Model", "Politics Model", "Policy Model", 
                            "Overall Model"))
@
