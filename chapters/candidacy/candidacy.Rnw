...
print(xtable(ab[-1,], label="tab:samplecomp", caption = "Comparing school districts that provided board election results to those that did not and are not included in the study." ), format.args = list(big.mark=","), include.rownames=TRUE, size="small")
...
stargazer(tax_model, poli_model, poly_model, uber_model, 
          omit = "fyear", font.size="small")

@

Let's look at within district variability. We can do this by fitting partial pooling 
models using a random-intercept framework \citep{gelhill} using the \texttt{lme4} 
...
# 
# tax_model <- glm(candidatesDV ~ primary2 + millrate + cum_attptsD + overlevy_ind_lag +
#                     log(politicaldivide) + perwhite + levy_chg +
#                    balance_member_lag +  median_income + locale3, 
#                  data=SBELEC, family="poisson")
# 
# summary(tax_model)
# # # Zero inflated
# # library(pscl)
# # summary(tax_model <- zeroinfl(candidatesDV ~  millrate + cum_attptsD + overlevy_ind_lag +
# #                     log(politicaldivide) + perwhite + levy_chg +
# #                    balance_member_lag +  median_income + locale3, 
# #                  data=SBELEC))
# 
...
we fit a simple longitudinal model of school districts. 
...
library(lme4)
simple_lmer <- glmer(contested2 ~ treatment +  
