...
In the end, \Sexpr{length(unique(dat$distid))} districts provided records for at 
least one year, and the median district provided 
\Sexpr{median(ddply(races, .(distid), summarize, out = length(unique(year)))[, 2])} 
years of records. Overall, records were provided on \Sexpr{nrow(races)} school 
board races involving \Sexpr{nrow(cand)} candidates in the state. For each election 
