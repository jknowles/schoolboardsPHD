...
demogsMMblq <- lmer(minBlaisLago ~ VAP_adjLOG + VAP_adjLOG_sq + 
                      ...
                      per_white_all + OOH_share + locale2 +  
                       RecallPolarization*treatment + treatment + (1|year) +
...
fiscalMMblq <- update(demogsMMblq, . ~ . + schoolMillShare + 
...
demogsMMblqBIN <- glmer(contestSer ~ VAP_adjLOG + VAP_adjLOG_sq + 
                      per65o + PerBachelorOrAbove + 
                      per_white_all + OOH_share + locale2 +  
                       RecallPolarization*treatment + treatment + (1|year) +
                       (1|distid), data = moddat, family = "binomial") 

politicMMblqBIN <- update(demogsMMblqBIN, . ~ . + incumShare +  overrideYesPer +  
                       partyDivisionRecent +
                    teachShareofVoters + fallTurnout + nraces) 


policyMMblqBIN <- update(demogsMMblqBIN, . ~ . + ADMIN_SHARE_COMP + NEW_SUP + 
                        NonPublicPupilPer + 
                   DEBT_ATTEMPT_CUMUL93 + OVERRIDE_ATTEMPT_CUMUL93)


causal1 <- glm(contestSer ~ turnoutLag1 + VAP_adjLOG + per_white_all + 
...
causal2 <- glm(contestSer ~ turnoutLag1 + VAP_adjLOG + per_white_all + 
