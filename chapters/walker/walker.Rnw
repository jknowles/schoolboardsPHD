...
#           Sweave = FALSE)
 ...
 apsrtable(lagModID, DIDmodID, FEmodID, se = "robust", stars = "default", 
          digits = 3, model.names = c("Lag", "DnD", "FE"), 
          caption = "Causal Estimates of Act 10 Period on Incumbent Defeat", 
          label = "tab:causID", notes = list(stars.note, omitNote), 
          Sweave = TRUE, 
           omitcoef=list("distid", 
                         expression(grep("distid", coefnames, value=TRUE))))
@
...
FEmodTurn <- lm(turnout ~ treatmentDiff + VAP_adjLOG_sq_lag + 
                  VAP_adjLOG_lag + per65o_lag + nraces_lag + OOH_share_lag + 
                  median_incomeLOG_lag + ImputedFlag + locale2_lag + 
                  PerBachelorOrAbove_lag +
...
FEmodRoll <- lm(turnoutDiff ~ treatmentDiff  + 
                   VAP_adjLOG_sq_lag + 
             VAP_adjLOG_lag + per65o_lag + nraces_lag + OOH_share_lag + 
               median_incomeLOG_lag + ImputedFlag +
               PerBachelorOrAbove_lag + fallTurnout_lag +
               distid, data = outdata)
