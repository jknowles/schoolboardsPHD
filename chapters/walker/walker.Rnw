...
glmNote <- c("Dependent variable is binomial. Coefficients are reported on 
             the logistic scale. ")
omitNote <- "Fixed effect model includes district fixed effects - not reported."
...
coefNames <- c("Intercept", "Lag SB Turnout", "Spring 2011", "Spring 2012", 
               "ln(VAP)", "$ln(VAP)^{2}$", "\\% Over 65", 
               "n races", "\\% OOH", "ln(Median Income)", "Imputed", "Districtwide", 
