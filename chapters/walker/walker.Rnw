...
<<blaisLagoplot, fig.cap="Lagged Competitiveness Across Years", out.height='3.18in'>>=
plotdf <- dist_turn[dist_turn$year > 2006, ]
# lg  <- function(x) c(NA, x[1:length(x)-1])
# lg2 <- function(x) c(NA, NA, x[2:length(x) -2])
plotdf <- as.data.table(plotdf)[, blaisLagoLag1:= lg(minBlaisLago), by = "distid"]
plotdf$minBlaisLago <- log(plotdf$minBlaisLago)
plotdf$blaisLagoLag1 <- log(plotdf$blaisLagoLag1)
plotdf$blaisLagoLag1[!is.finite(plotdf$blaisLagoLag1)] <- -4
plotdf$minBlaisLago[!is.finite(plotdf$minBlaisLago)] <- -4
plotdf <- as.data.frame(plotdf)
plotdf <- na.omit(plotdf[, c("blaisLagoLag1", "minBlaisLago", "year")])
...
<<turnoutPlot1, fig.cap="Lagged School Board Turnout Across Years", out.height='3.18in'>>=
plotdf <- dist_turn[dist_turn$year >= 2006, ]
plotdf <- as.data.table(plotdf)[, turnoutLag1:= lg(turnout), by = "distid"]
plotdf$turnout <- log(plotdf$turnout)
plotdf$turnoutLag1 <- log(plotdf$turnoutLag1)
plotdf <- as.data.frame(plotdf)
plotdf <- na.omit(plotdf[, c("turnoutLag1", "turnout", "year")])
plotdf <- plotdf[plotdf$year >= 2007,]
qplot(turnoutLag1, turnout, data = plotdf, group = year) + 
  facet_wrap(~year) + geom_smooth(method = 'lm') + 
  geom_abline(intercept = -1.3192, slope = 0.2615, linetype = 2) +
  scale_x_continuous(limits = c(-3, 0)) +
  scale_y_continuous(limits = c(-3, 0)) +
  theme_dpi(base_size=10) +
  labs(x= "School Board Turnout Lag (log) ", y = "School Board Turnout (log)")
@

<<turnoutPlot2, fig.cap="Lagged Spring Rolloff Across Years", out.height='3.18in'>>=
plotdf <- dist_turn[dist_turn$year > 2006, ]
plotdf <- as.data.table(plotdf)[, turnoutDiffLag1:= lg(turnoutDiff), by = "distid"]
plotdf <- as.data.frame(plotdf)
plotdf <- na.omit(plotdf[, c("turnoutDiffLag1", "turnoutDiff", "year")])
plotdf <- plotdf[plotdf$year > 2008,]

qplot(turnoutDiffLag1, turnoutDiff, data = plotdf, group = year) + 
  facet_wrap(~year) + geom_smooth(method = 'rlm') + 
  geom_abline(intercept = .02617, slope = 0.33217, linetype = 2) +
  theme_dpi(base_size=10) +
  labs(x= "School Board Rolloff Lag (log) ", y = "School Board Rolloff (log)")
@
...
<<pairsPlot, fig.cap="Pairs Plot of Demographic Variables and Turnout in 2012", fig.height = 8, fig.width=8>>=
library(GGally)
dist_turn$median_incomeLOG <- log(dist_turn$median_income)
pairLabels <- c("Turnout", "% Incumbents", "% 65 Older", "% Bach. +", "OOH %", 
                "Median Income", "Log VAP")
tmpdf <- dist_turn[dist_turn$turnout < 0.95 & 
                            dist_turn$year == 2012 & 
                            !is.na(dist_turn$turnout), ]
tmpdf <- tmpdf[, c("turnout", "incumShare", "per65o", "PerBachelorOrAbove", 
