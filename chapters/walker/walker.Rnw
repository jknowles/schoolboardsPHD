...
# 
# MermodContest <- glmer(contestMinor ~ contestMinor_lag * treatmentDiff + 
#                          VAP_adjLOG_sq_lag + incumShare_lag + 
#              VAP_adjLOG_lag + per65o_lag + nraces_lag + OOH_share_lag +  
#                PerBachelorOrAbove_lag + locale2_lag + (1|distid), 
#              data = outdata, family = "binomial", control = ctrl)

# causMinorConTab <- apsrtable(lagModcontest, DIDmodContest, 
#                              MermodContest, se = "robust", 
...
#           Sweave = TRUE)
glmNote <- c("Dependent variable is binomial. Coefficients are reported on the logistic scale. ")
...
# MermodContest2 <- glmer(contestSer ~ contestSer_lag * treatmentDiff + 
#                           VAP_adjLOG_sq_lag + incumShare_lag +
#              VAP_adjLOG_lag + per65o_lag + nraces_lag + OOH_share_lag +  
#                PerBachelorOrAbove_lag + locale2_lag + (1|distid), 
#              data = outdata, family = "binomial", control = ctrl)
...
lagModBL <- lm(minBlaisLago ~ minBlaisLago_lag + treatmentDiff + 
                 VAP_adjLOG_sq_lag + incumShare_lag + 
             VAP_adjLOG_lag + per65o_lag + nraces_lag + OOH_share_lag + 
               PerBachelorOrAbove_lag + 
...
DIDmodBL <- lm(minBlaisLago ~ minBlaisLago_lag * treatmentDiff + 
                 VAP_adjLOG_sq_lag + incumShare_lag + 
             VAP_adjLOG_lag + per65o_lag + nraces_lag + OOH_share_lag +  
               PerBachelorOrAbove_lag +
             locale2_lag, data = outdata)
...
merModBL <- lmer(minBlaisLago ~ minBlaisLago_lag * treatmentDiff +
                   VAP_adjLOG_sq_lag + incumShare_lag + 
             VAP_adjLOG_lag + per65o_lag + nraces_lag + OOH_share_lag +  
               PerBachelorOrAbove_lag +
...
lagModID <- glm(incDefeats ~ incDefeats_lag + treatmentDiff + VAP_adjLOG_sq_lag + 
             VAP_adjLOG_lag + per65o_lag + nraces_lag + OOH_share_lag + 
...
DIDmodID <- glm(incDefeats ~ incDefeats_lag * treatmentDiff + VAP_adjLOG_sq_lag + 
             VAP_adjLOG_lag + per65o_lag + nraces_lag + OOH_share_lag +  
               PerBachelorOrAbove_lag + incumShare_lag +
             locale2_lag, data = outdata, family = "binomial")
...
# merModID <- glmer(incDefeats ~ incDefeats_lag * treatmentDiff + VAP_adjLOG_sq_lag + 
#              VAP_adjLOG_lag + per65o_lag + nraces_lag + OOH_share_lag +  
#                PerBachelorOrAbove_lag + incumShare_lag +
...
outdata <- outdata[, c("turnout", "ImputedFlag", "distid", "year")]
outdata <- merge(outdata, preTreat, by.y = c("distid_lag", "year_lag"), 
                 by.x = c("distid", "year"))
outdata$treatmentDose <- ifelse(outdata$year >= 2011, abs(outdata$year - 2013), 0)
outdata$treatmentFactor <- ifelse(outdata$year >= 2011, 1, 0)
...
lagModturn <- lm(turnout ~ turnout_lag + treatmentDiff + VAP_adjLOG_sq_lag + 
             VAP_adjLOG_lag + per65o_lag + nraces_lag + OOH_share_lag + 
               median_incomeLOG_lag + ImputedFlag +
...
DIDmodturn <- lm(turnout ~ turnout_lag * treatmentDiff + VAP_adjLOG_sq_lag + 
             VAP_adjLOG_lag + per65o_lag + nraces_lag + OOH_share_lag + 
               median_incomeLOG_lag+ ImputedFlag + 
             locale2_lag + PerBachelorOrAbove_lag + fallTurnout_lag, 
             data = outdata)
...
merModturn <- lmer(turnout ~ turnout_lag * treatmentDiff + VAP_adjLOG_sq_lag + 
             VAP_adjLOG_lag + per65o_lag + nraces_lag + OOH_share_lag + 
               median_incomeLOG_lag + ImputedFlag +
             locale2_lag + PerBachelorOrAbove_lag + fallTurnout_lag + 
               (1|distid), data = outdata)

FEmodTurn <- lm(turnout ~ treatmentDiff + nraces_lag + ImputedFlag + 
...
outdata <- outdata[, c("turnoutDiff", "ImputedFlag", "distid", "year")]
outdata <- merge(outdata, preTreat, by.y = c("distid_lag", "year_lag"), 
                 by.x = c("distid", "year"))
...
#           label = "tab:causRoll", notes = list(stars.note), digits = 3, 
#           Sweave = TRUE)
...
simp1 <- glm(contestSer ~ contestSer_lag + treatmentDiff * RecallPolarization_lag, 
             data = outdata, family = 'binomial')
simp1$se <- sqrt(diag(get_CL_vcov(simp1, outdata$distid)))

simp2 <- glm(contestMinor ~ contestMinor_lag + treatmentDiff * RecallPolarization_lag, 
             data = outdata, family = 'binomial')
simp2$se <- sqrt(diag(get_CL_vcov(simp2, outdata$distid)))

simp3 <- lm(minBlaisLago ~ minBlaisLago_lag + treatmentDiff * RecallPolarization_lag, 
             data = outdata)
simp3$se <- sqrt(diag(get_CL_vcov(simp3, outdata$distid)))

simp4 <- lm(turnout ~ turnout_lag + treatmentDiff * RecallPolarization_lag + 
              ImputedFlag, 
             data = outdata)
simp4$se <- sqrt(diag(get_CL_vcov(simp4, outdata$distid)))

simp5 <- lm(turnoutDiff ~ turnoutDiff_lag + treatmentDiff * RecallPolarization_lag + 
              ImputedFlag, 
             data = outdata)
simp5$se <- sqrt(diag(get_CL_vcov(simp5, outdata$distid)))

simp6 <- glm(incDefeats ~ incDefeats_lag + treatmentDiff * RecallPolarization_lag, 
             data = outdata, family = "binomial")
...
full1 <- glmer(contestMinor ~ contestMinor_lag + treatmentDiff + 
             ...
             VAP_adjLOG_lag + per65o_lag + nraces_lag + OOH_share_lag +  
               ...
               treatmentDiff * WERC_str_lag + (1|distid), 
             data = outdata, family = "binomial", control = ctrl)
...
full1 <- glmer(contestMinor ~ contestMinor_lag + treatmentDiff + VAP_adjLOG_sq_lag + 
             VAP_adjLOG_lag + per65o_lag + nraces_lag + OOH_share_lag +  
               PerBachelorOrAbove_lag + locale2_lag + 
               treatmentDiff * RecallTurnout_lag + (1|distid), 
             data = outdata, family = "binomial", control = ctrl)
