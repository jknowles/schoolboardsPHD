...
not strongly responsive to social group and partisan influences. Thus, 
...
were focused across the state. While I still contend that overall the decade from 
...
<<corMat, fig.cap="Correlation Matrix of Demographic Variables and Turnout Across Sample", fig.height = 8, fig.width=8>>=
...
certification elections, and won \Sexpr{iNum(sum(wercDatTeachers$wins_werc))}. This 
...
organize. On average, districts saw \Sexpr{iPer(mean(wercDatTeachers$best_margin_werc)*100)} margins of votes in favor of certification.  
...
<<ivPlot3, fig.cap = "Polarization and WERC Elections", eval=FALSE>>=
...
outdata <- outdata[, c("turnout", "ImputedFlag", "distWidemix", "distid", "year")]
outdata <- merge(outdata, preTreat, by.y = c("distid_lag", "year_lag"), 
                 by.x = c("distid", "year"))
outdata$treatmentDose <- ifelse(outdata$year >= 2011, abs(outdata$year - 2013), 0)
outdata$treatmentFactor <- ifelse(outdata$year >= 2011, 1, 0)
...
FEmodTurn <- lm(turnout ~ treatmentDiff + nraces_lag + ImputedFlag + distWidemix +
...
simp4 <- lm(turnout ~ treatmentDiff * RecallPolarization_lag + 
              ImputedFlag +  distWidemix + turnout_lag, 
             data = outdata)
...
caption = "Mediation Estimates of Polarization on Board Elections - Multilevel Models",           label = "tab:polarMedHLM", notes = list(stars.note), 
        Sweave = FALSE, 
          stars = "default", size = "footnotesize")
@
...
recallTurnMedTable <- apsrtable(full1, full3, full4, full5, full6, 
          ...
          Sweave = FALSE, size = "footnotesize")
@
