...
# Bigger the number, less polarization there is
qplot(RecallPolarization, minBlaisLago, data = moddat[moddat$turnout < 80,], 
      color = factor(contestSer)) + 
  facet_wrap(~year) + geom_smooth(method = 'lm', aes(group = 1)) +
  scale_y_continuous() +
  theme_dpi()
...
politicMMserBIN <- update(demogsMMserBIN, . ~ . + incumShare +  overrideYesPer +  
                       partyDivisionRecent +
...
<<walkerplotT, fig.cap="Polarization and Turnout Before and After Act 10">>=
