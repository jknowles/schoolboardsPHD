...
moddat <- dist_turn[, c("turnout", "turnoutLag2", "turnoutLag1","year", 
...
penalized estimation to reduce the influence of outliers. The results for these 
alternative specifications are displayed alongisde the initial model in Table 
\ref{tab:lagmods}. 
...
forming in school board elections. Control variables 
...
other parameters and highly inflated coefficients for other parameters. 

%Citation from Gelman about preference of partial pooling

Indeed, the district estimates themselves are problematic. With over 300 
...
in order to avoid overfit. We should also fit some better models. Partial pooling 
...
are highly predictive of current turnout. This suggests that voters in school board 
...
closeRLM2 <- ols(turnout ~ CLOSE + fallTurnout + VAP_adj + per_white_all + 
                    per65o + PerBachelorOrAbove + OOH_share + PCI + districtwide +
                    factor(year),
              data = moddat, x= TRUE, y = TRUE)
...
closeRLM3 <- ols(turnout ~ CLOSE + turnoutLag2 + fallTurnout + VAP_adj + per_white_all +
                   per65o + PerBachelorOrAbove + OOH_share + PCI + districtwide +
                  factor(distid), 
              data = moddat, x = TRUE, y = TRUE)
...
<<model4, results='asis'>>=
mixedMod2 <- lmer(turnout ~ CLOSE + fallTurnout + VAP_adj + per_white_all +
                   per65o + PerBachelorOrAbove + OOH_share + PCI + districtwide +
                  (1|distid), 
              data = moddat, REML = TRUE)
