...
I now review the results for the four models above. First, I look for evidence 
of these models providing predictive power on turnout in the spring school board 
races. Then I apply these same models to the rolloff of voters in the spring 
election. 

% Autocorrelation - habit forming
% Electorate size - game theory, value of vote
% Electorate size * competitiveness - pivotal voter
% Social groups -- unionization and partisanship
...
naive1$se <- sqrt(diag(vcov(naive1)))
...
dumbRLMbest <- ols(turnout ~ turnoutLag2 *VAP_adjLOG + fallTurnout + VAP_adjLOG +
                     VAP_adjLOG_sq + per_white_all + per65o + PerBachelorOrAbove +
                     OOH_share + median_incomeLOG + districtwide + 
               minBlaisLago + factor(year), data = moddat, x= TRUE, y = TRUE)

dumbRLMbest$se <- sqrt(diag(vcov(robcov(dumbRLMbest, cluster = moddat$distid))))


dumbRLM2 <- ols(turnout ~ turnoutLag2 + fallTurnout + VAP_adjLOG + VAP_adjLOG_sq + 
                  per_white_all + per65o + PerBachelorOrAbove + OOH_share + 
                  median_incomeLOG + districtwide + minBlaisLago + 
...
apsrtable(naive1, dumbRLM1, dumbRLM1.2, dumbRLMbest, dumbRLM2, se="robust", 
          label="tab:lagmods", 
          model.names=c("Naive OLS", "Simple RLM", "RLM + Year FE", "Interaction", 
                        "RLM + District FE"), 
          caption = "Models of Lagged Turnout in School Board Elections",
          notes = list("Standard errors clustered at the school district except in model 1.", 
                       stars.note, 
                       "Year and district fixed effects omitted."),
...
mixedMod1 <- lmer(turnout ~ turnoutLag2 + fallTurnout + VAP_adjLOG + 
                    per_white_all + turnoutLag2 * VAP_adjLOG + 
                       per65o + PerBachelorOrAbove + OOH_share + median_incomeLOG + 
               districtwide + 
               minBlaisLago + (1|distid), 
              data = moddat, REML = FALSE)

mixedMod1b <- lmer(turnout ~ turnoutLag2 + fallTurnout + VAP_adjLOG + 
                    per_white_all + turnoutLag2 * VAP_adjLOG + 
                       per65o + PerBachelorOrAbove + OOH_share + median_incomeLOG + 
               districtwide + 
...
apsrtable(mixedMod1b, caption = "Partial Pooling Model of Lagged Turnout",
          model.names = c("Partial Pooled"), 
            label = "tab:meTurnout",  
           notes = list("Group level intercepts are not shown.", 
                       stars.note), 
...
moddat <- dist_turn[, c("turnout", "turnoutLag2", "year", "fallTurnout", 
                        ...
                        "OOH_share", "median_incomeLOG", 
                        "CLOSE", "districtwide", "contest", "minBlaisLago")]
moddat <- na.omit(moddat)
moddat <- moddat[order(moddat$distid, moddat$year),]
moddat$distid <- factor(moddat$distid)
moddat$year <- factor(moddat$year)
...
closeRLM2 <- ols(turnout ~ VAP_adjLOG + per_white_all + 
                  per65o + PerBachelorOrAbove + OOH_share + median_incomeLOG +
                   districtwide + minBlaisLago + factor(year),
              data = moddat, x= TRUE, y = TRUE)
closeRLM2$se <- sqrt(diag(vcov(robcov(closeRLM2, cluster = moddat$distid))))
closeRLM3 <- ols(turnout ~ VAP_adjLOG + per_white_all + 
                  per65o + PerBachelorOrAbove + OOH_share + median_incomeLOG +
                   districtwide + minBlaisLago + factor(distid), 
              data = moddat, x = TRUE, y = TRUE)
closeRLM3$se <- sqrt(diag(vcov(robcov(closeRLM3, cluster = moddat$distid))))
closeRLM4 <- ols(turnout ~ VAP_adjLOG + minBlaisLago*VAP_adjLOG + per_white_all + 
                  per65o + PerBachelorOrAbove + OOH_share + median_incomeLOG +
                   districtwide + minBlaisLago + factor(year), 
              data = moddat, x = TRUE, y = TRUE)
closeRLM4$se <- sqrt(diag(vcov(robcov(closeRLM4, cluster = moddat$distid))))

apsrtable(closeRLM, closeRLM2, closeRLM3, closeRLM4, se="robust", 
          label="tab:closemods", 
          model.names=c("Simple RLM", "RLM + Year FE", "RLM + District FE", "Interact + Year FE"), 
...
\begin{tiny}
<<model4, results='asis'>>=
mixedMod2 <- lmer(turnout ~  minBlaisLago + VAP_adjLOG + 
...
mixedMod2b <- lmer(turnout ~ minBlaisLago + VAP_adjLOG + 
                     minBlaisLago*VAP_adjLOG + per_white_all + 
                     per65o + PerBachelorOrAbove + OOH_share + 
                     median_incomeLOG + districtwide + 
...
moddat <- dist_turn[, c("turnout", "turnoutLag2", "year", "fallTurnout", 
                        ...
                        "CLOSE", "teachShareofVoters", "districtwide", 
...
teachRLM3 <- ols(turnout ~ teachShareofVoters + fallTurnout + minBlaisLago + 
...
apsrtable(teachRLM, teachRLM2, teachRLM3, se="robust", label="tab:teachmods", 
          model.names=c("Simple RLM", "RLM + Year FE", "RLM + District FE"), 
...
mixedMod3 <- lmer(turnout ~ teachShareofVoters + fallTurnout + minBlaisLago + 
                    VAP_adjLOG + per_white_all + teachShareofVoters * VAP_adjLOG + 
                   per65o + PerBachelorOrAbove + OOH_share + median_incomeLOG +
                     districtwide + contest+ (1|distid), 
              data = moddat, REML = TRUE)

mixedMod3b <- lmer(turnout ~ teachShareofVoters + fallTurnout + minBlaisLago + 
                    VAP_adjLOG + per_white_all + teachShareofVoters * VAP_adjLOG + 
                   per65o + PerBachelorOrAbove + OOH_share + median_incomeLOG +
                     districtwide + contest+ (1|year) + (1|distid), 
              data = moddat, REML = TRUE)

apsrtable(mixedMod3b, caption = "Partial Pooling Model of Staff Influence",
          model.names = c("Partial Pooled"),
...
rm(dist.tmp, dat, cand, VAP_dist, distAttr, dvp, errors, govTurn, presTurn, races, 
   ...
   naive1, naive2, mixedMod1, mixedMod2, mixedMod3)
@
...
<<ReviewTab, results='asis'>>=

apsrtable(mixedMod1b, mixedMod2b, mixedMod3b, 
          caption = "Partial Pooling Model of Post-Election Polarization",
          float = 'longtable', 
          model.names = c("Lagged", "Competitive", "Blank"),
            label = "tab:modReview", notes = list(tabnote, stars.note), 
          Sweave = FALSE)
