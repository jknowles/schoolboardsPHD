...
mixedMod1 <- lmer(turnout ~ turnoutLag2 +  VAP_adjLOG + 
                    per_white_all + 
                       per65o + PerBachelorOrAbove + OOH_share + median_incomeLOG + 
               districtwide + 
...
mixedMod1b <- lmer(turnout ~ turnoutLag2 + fallTurnout + VAP_adjLOG + VAP_adjLOG_sq +
                    per_white_all + 
                       per65o + PerBachelorOrAbove + OOH_share + median_incomeLOG + 
               districtwide + 
               minBlaisLago + (1|year) + (1|distid), 
...
# # boo01 <- bootMer(mixedMod1, mySumm, nsim = 500) 
# # bCI.1 <- boot.ci(boo01, index=2, type=c("norm", "basic", "perc"))# beta

tabnote <- "Model includes both year and district random effects modeled as intercepts."

apsrtable(mixedMod1, mixedMod1b, caption = "Partial Pooling Model of Lagged Turnout",
          model.names = c("No Fall Turnout", "Partial Pooled"), 
            label = "tab:meTurnout",  
...
m.slope <- update(mixedMod1b, . ~ . - (1|year))
m0 <- update(mixedMod1b, . ~ . - (1|distid))
exactRLRT(m.slope, mixedMod1b, m0)
...
\begin{tiny}
<<model4, results='asis'>>=
mixedMod2 <- lmer(turnout ~  minBlaisLago + VAP_adjLOG + VAP_adjLOG_sq +
...
mixedMod2b <- lmer(turnout ~ minBlaisLago + VAP_adjLOG +  VAP_adjLOG_sq + 
                     fallTurnout +
                     minBlaisLago*VAP_adjLOG + per_white_all + 
                     per65o + PerBachelorOrAbove + OOH_share + 
...
# summary(mixedMod2)
apsrtable(mixedMod2b, caption = "Partial Pooling Model for Competitiveness",
...
exactLRT(m = refitML(mixedMod2), m0 = m0)

# Test for year specific slopes
m.slope <- update(mixedMod2b, . ~ . - (1|year))
m0 <- update(mixedMod2b, . ~ . - (1|distid))
exactRLRT(m.slope, mixedMod2b, m0)
@
...
\begin{tiny}
<<mixedInfMod, results='asis'>>=
mixedMod3 <- lmer(turnout ~ teachShareofVoters +  minBlaisLago +  
                    VAP_adjLOG + per_white_all + per65o + PerBachelorOrAbove +
                   OOH_share + median_incomeLOG + districtwide + contest + 
                   teachShareofVoters * VAP_adjLOG + (1|distid), 
              data = moddat, REML = TRUE)

mixedMod3b <- lmer(turnout ~ teachShareofVoters + fallTurnout + minBlaisLago +  VAP_adjLOG_sq +
                    VAP_adjLOG + per_white_all + per65o + PerBachelorOrAbove +
                   OOH_share + median_incomeLOG + districtwide + contest + 
                   teachShareofVoters * VAP_adjLOG + (1|year) + (1|distid), 
              data = moddat, REML = TRUE)

apsrtable(mixedMod3, mixedMod3b, caption = "Partial Pooling Model of Staff Influence",
...
exactLRT(m = refitML(mixedMod3), m0 = m0)
# Test for year specific slopes
m.slope <- update(mixedMod3b, . ~ . - (1|year))
m0 <- update(mixedMod3b, . ~ . - (1|distid))
exactRLRT(m.slope, mixedMod3b, m0)
@
...
moddat <- dist_turn[, c("turnout", "turnoutLag2", "turnoutDiff","year", 
                        "fallTurnout", "VAP_adjLOG", "distid", 
                        "per_white_all", "per65o", "PerBachelorOrAbove", 
                        "OOH_share", "median_incomeLOG", "minBlaisLago", 
                        "CLOSE", "teachShareofVoters", "districtwide", "contest", 
                        "ImputedFlag")]
...
undvote1 <- lmer(turnoutDiff ~ turnoutLag2 * VAP_adjLOG + per_white_all + 
                   per65o + PerBachelorOrAbove + OOH_share + median_incomeLOG + 
                   minBlaisLago + 
                   contest + teachShareofVoters + (1|year) + (1|distid),
                   data = moddat)

undvote2 <- lmer(turnoutDiff ~ turnoutLag2 * VAP_adjLOG + per_white_all + 
                   per65o + PerBachelorOrAbove + OOH_share + median_incomeLOG + 
                   minBlaisLago + 
                   contest + teachShareofVoters + (1|distid),
                   data = moddat)

m0 <- lm(turnoutDiff ~ turnoutLag2 * VAP_adjLOG + per_white_all + 
                   per65o + PerBachelorOrAbove + OOH_share + median_incomeLOG + 
                    minBlaisLago  + contest + teachShareofVoters, 
         data = moddat) # fit the null model

undvote3 <- lmer(turnoutDiff ~ turnoutLag2 + VAP_adjLOG + per_white_all + 
                   per65o + PerBachelorOrAbove + OOH_share + median_incomeLOG + 
                   minBlaisLago + teachShareofVoters * VAP_adjLOG + 
                   contest + teachShareofVoters + (1|distid),
                   data = moddat)
