...
qplot(teachShareofVoters, turnout, 
      data = na.omit(dist_turn[dist_turn$year > 2004 & dist_turn$turnout < 0.9 & dist_turn$teachShareofVoters >0 ,])) + 
...
qplot(turnout, data = na.omit(dist_turn[dist_turn$turnout < 0.95,]), fill = CLOSE, 
      color = I("gray90")) +  
  theme_dpi() + 
...
qplot(abs(fallTwoPartyShareDem-.5), turnout, data = na.omit(dist_turn[dist_turn$turnout < 0.95,])) + facet_wrap(~year) + 
  geom_smooth(se = TRUE, method = 'rlm') + theme_dpi() + 
  scale_x_continuous() + 
  scale_y_continuous(label = percent) + 
...
ggpairs(na.omit(dist_turn[dist_turn$turnout < 0.95 & dist_turn$year == 2012,]), 
        columns = c(25, 41:43, 46), 
...
moddat <- dist_turn[, c("turnout", "turnoutLag2", "turnoutLag1","year", "contest",
                        "fallTurnout", "VAP_adj", "distid", "districtwide",
...
naive1 <- lm(turnout ~ turnoutLag2 + fallTurnout + VAP_adj + per_white_all + 
                       per65o + PerBachelorOrAbove + OOH_share + median_income + districtwide + 
               contest,
             data = moddat)
...
library(rms)
library(devtools)
install_github("jknowles/apsrtable", ref = "dev", subdir = "pkg")
library(apsrtable)
moddat <- moddat[moddat$outflag <= 5 & moddat$outflag >= -5,]
dd <<- datadist(moddat)
options(datadist="dd")

dumbRLM1 <- ols(turnout ~ turnoutLag2 + fallTurnout + VAP_adj + per_white_all + 
                  per65o + PerBachelorOrAbove + OOH_share + median_income + districtwide + 
...
dumbRLM1.2 <- ols(turnout ~ turnoutLag2 + fallTurnout + VAP_adj + per_white_all + 
                    per65o + PerBachelorOrAbove + OOH_share + median_income + districtwide +
                    contest + factor(year), data = moddat, x= TRUE, y = TRUE)

dumbRLM1.2$se <- sqrt(diag(vcov(robcov(dumbRLM1.2, cluster = moddat$distid))))

dumbRLMbest <- ols(turnout ~ turnoutLag2 + turnoutLag2 * VAP_adj + 
                     fallTurnout + VAP_adj + per_white_all + 
                    per65o + PerBachelorOrAbove + OOH_share + median_income + districtwide +
                    contest + factor(year), data = moddat, x= TRUE, y = TRUE)
...
dumbRLM2 <- ols(turnout ~ turnoutLag2 + fallTurnout + VAP_adj + per_white_all +
                   per65o + PerBachelorOrAbove + OOH_share + median_income + districtwide +
...
naive2 <- lm(turnout ~ turnoutLag1 + fallTurnout + VAP_adj + per_white_all + 
                       per65o + PerBachelorOrAbove + OOH_share + median_income + districtwide + 
               contest,
             data = moddat)

turn21 <- ols(turnout ~ turnoutLag1 + fallTurnout + VAP_adj + per_white_all + 
                  per65o + PerBachelorOrAbove + OOH_share + median_income + districtwide + 
                contest, 
              data = moddat, x = TRUE, y = TRUE)
...
turn22 <- ols(turnout ~ turnoutLag1 + fallTurnout + VAP_adj + per_white_all + 
                    per65o + PerBachelorOrAbove + OOH_share + median_income + districtwide +
                    contest + factor(year),
              data = moddat, x= TRUE, y = TRUE)
...
turn23 <- ols(turnout ~ turnoutLag1 + fallTurnout + VAP_adj + per_white_all +
                   per65o + PerBachelorOrAbove + OOH_share + median_income + districtwide +
                  contest + factor(distid), 
              data = moddat, x = TRUE, y = TRUE)
...
mixedMod1 <- lmer(turnout ~ turnoutLag2 + turnoutLag2 * VAP_adj +  fallTurnout + 
                    VAP_adj + per_white_all +
                   per65o + PerBachelorOrAbove + OOH_share + 
                   median_income + districtwide +  contest + (1|distid), 
              data = moddat, REML = FALSE)

mixedMod1b <- lmer(turnout ~ turnoutLag2 + turnoutLag2 * VAP_adj +  fallTurnout + 
                    VAP_adj + per_white_all +
                   per65o + PerBachelorOrAbove + OOH_share + 
                   median_income + districtwide +  contest + (1|year) + (1|distid), 
              data = moddat, REML = FALSE)
...
library(RLRsim)
m0 <- lm(turnout ~ turnoutLag2 + turnoutLag2 * VAP_adj +  fallTurnout + 
                    VAP_adj + per_white_all +
                   per65o + PerBachelorOrAbove + OOH_share + 
                   median_income + districtwide +  contest, 
...
<<model3, results ='asis'>>=
moddat <- dist_turn[, c("turnout", "turnoutLag2", "year", "fallTurnout", "VAP_adj", "distid", 
                        "per_white_all", "per65o", "PerBachelorOrAbove", "OOH_share", "median_income", 
...
closeRLM <- ols(turnout ~ CLOSE + VAP_adj + per_white_all + 
                  per65o + PerBachelorOrAbove + OOH_share + median_income + districtwide + 
                  contest, 
              data = moddat, x = TRUE, y = TRUE)
...
closeRLM2 <- ols(turnout ~ CLOSE + VAP_adj + per_white_all + 
                    per65o + PerBachelorOrAbove + OOH_share + median_income + districtwide +
                    contest + factor(year),
              data = moddat, x= TRUE, y = TRUE)
...
closeRLM3 <- ols(turnout ~ CLOSE + VAP_adj + per_white_all +
                   per65o + PerBachelorOrAbove + OOH_share + median_income + districtwide +
                  contest + factor(distid), 
              data = moddat, x = TRUE, y = TRUE)
...
closeRLM4 <- ols(turnout ~ CLOSE + VAP_adj + CLOSE*VAP_adj + per_white_all +
                   per65o + PerBachelorOrAbove + OOH_share + median_income + districtwide +
                  contest + factor(year), 
              data = moddat, x = TRUE, y = TRUE)
...
<<model4, results='asis'>>=
mixedMod2 <- lmer(turnout ~ CLOSE + VAP_adj + CLOSE*VAP_adj + per_white_all +
                   per65o + PerBachelorOrAbove + OOH_share + median_income + districtwide +
                    contest + (1|distid), 
              data = moddat, REML = TRUE)

mixedMod2b <- lmer(turnout ~ CLOSE + VAP_adj + CLOSE*VAP_adj + per_white_all +
                   per65o + PerBachelorOrAbove + OOH_share + median_income + districtwide +
...
m0 <- lm(turnout ~ CLOSE + VAP_adj + CLOSE*VAP_adj + per_white_all +
                   per65o + PerBachelorOrAbove + OOH_share + median_income + districtwide +
...
undvote1 <- lmer(turnoutDiff ~ turnoutLag2 * VAP_adj + per_white_all + 
                   per65o + PerBachelorOrAbove + OOH_share + median_income + CLOSE + 
                   contest + teachShareofVoters + (1|year) + (1|distid),
                   data = moddat)
undvote2 <- lmer(turnoutDiff ~ turnoutLag2 * VAP_adj + per_white_all + 
                   per65o + PerBachelorOrAbove + OOH_share + median_income + CLOSE + 
                   contest + teachShareofVoters + (1|distid),
                   data = moddat)

m0 <- lm(turnoutDiff ~ turnoutLag2 + VAP_adj + per_white_all + 
                   per65o + PerBachelorOrAbove + OOH_share + median_income + CLOSE + 
...
causal1 <- ols(turnout ~ turnoutLag2 + fallTurnout + VAP_adj + per_white_all + 
               per65o + PerBachelorOrAbove + OOH_share + median_income + CLOSE + 
...
causal2 <- ols(turnout ~ turnoutLag2 + fallTurnout + topTicketTurnoutSpring + 
                 VAP_adj + per_white_all + 
               per65o + PerBachelorOrAbove + OOH_share + median_income + CLOSE + 
               districtwide + contest + RecallPolarization*treatment + 
               treatment + turnout_overall_werc, 
...
mixedModCaus <- lmer(turnout ~ turnoutLag2 + fallTurnout + topTicketTurnoutSpring + 
                 VAP_adj + per_white_all + 
               per65o + PerBachelorOrAbove + OOH_share + median_income + CLOSE + 
               districtwide + contest + RecallPolarization*treatment + 
...
mixedModCaus2 <- lmer(turnoutDiff*100 ~ turnoutLag2 + fallTurnout +  
                        topTicketTurnoutSpring + VAP_adj + per_white_all + 
               per65o + PerBachelorOrAbove + OOH_share + median_income + CLOSE + 
               districtwide + contest + RecallPolarization*treatment + 
               treatment + turnout_overall_werc + (1|distid) + (1|year), 
              data = moddat)
