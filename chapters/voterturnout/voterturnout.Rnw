...
closeRLM <- ols(turnout ~ CLOSE + log(VAP_adj) + per_white_all + 
                  per65o + PerBachelorOrAbove + OOH_share + log(PCI), 
              data = moddat, x = TRUE, y = TRUE)
...
closeRLM2 <- ols(turnout ~ CLOSE + fallTurnout + log(VAP_adj) + per_white_all + 
                    per65o + PerBachelorOrAbove + OOH_share + log(PCI) +
                    factor(year),
              data = moddat, x= TRUE, y = TRUE)
...
closeRLM3 <- ols(turnout ~ CLOSE + turnoutLag2 + fallTurnout + log(VAP_adj) + per_white_all +
                   per65o + PerBachelorOrAbove + OOH_share + log(PCI) +
                  factor(distid), 
              data = moddat, x = TRUE, y = TRUE)
...
<<model4, results='markup'>>=
mixedMod2 <- lmer(turnout ~ CLOSE + turnoutLag2 + fallTurnout + log(VAP_adj) + per_white_all +
                   per65o + PerBachelorOrAbove + OOH_share + log(PCI) +
                  (1|distid), 
              data = moddat, REML = TRUE)
