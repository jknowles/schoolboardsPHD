...
dumbRLM1 <- ols(turnout ~ turnoutLag2 + fallTurnout + VAP_adjLOG + VAP_adjLOG_sq + 
               ...
               districtwide + 
               contest, data = moddat, x = TRUE, y = TRUE)
...
dumbRLM1.2 <- ols(turnout ~ turnoutLag2 + fallTurnout + VAP_adjLOG + VAP_adjLOG_sq + 
                    per_white_all + 
                       per65o + PerBachelorOrAbove + OOH_share + median_incomeLOG + 
               districtwide + 
               contest + factor(year), data = moddat, x= TRUE, y = TRUE)

dumbRLM1.2$se <- sqrt(diag(vcov(robcov(dumbRLM1.2, cluster = moddat$distid))))

dumbRLMbest <- ols(turnout ~ turnoutLag2 *VAP_adjLOG + fallTurnout + VAP_adjLOG + VAP_adjLOG_sq +
                     per_white_all + 
                       per65o + PerBachelorOrAbove + OOH_share + median_incomeLOG + 
               districtwide + 
               contest + factor(year), data = moddat, x= TRUE, y = TRUE)
...
closeRLM <- ols(turnout ~ CLOSE + VAP_adjLOG + per_white_all + 
                  per65o + PerBachelorOrAbove + OOH_share + median_incomeLOG + districtwide + 
                  contest, 
              data = moddat, x = TRUE, y = TRUE)
...
closeRLM2 <- ols(turnout ~ CLOSE + VAP_adjLOG + per_white_all + 
                  per65o + PerBachelorOrAbove + OOH_share + median_incomeLOG + districtwide + 
                  contest+ factor(year),
              data = moddat, x= TRUE, y = TRUE)
...
closeRLM3 <- ols(turnout ~CLOSE + VAP_adjLOG + per_white_all + 
                  per65o + PerBachelorOrAbove + OOH_share + median_incomeLOG + districtwide + 
                  contest + factor(distid), 
              data = moddat, x = TRUE, y = TRUE)
...
closeRLM4 <- ols(turnout ~ CLOSE + VAP_adjLOG + CLOSE*VAP_adjLOG + per_white_all + 
                  per65o + PerBachelorOrAbove + OOH_share + median_incomeLOG + districtwide + 
                  contest + factor(year), 
              data = moddat, x = TRUE, y = TRUE)
...
apsrtable(closeRLM, closeRLM2, closeRLM3, closeRLM4, se="robust", label="tab:closemods", 
...
<<model4, results='asis'>>=
mixedMod2 <- lmer(turnout ~  CLOSE + VAP_adjLOG + CLOSE*VAP_adjLOG + per_white_all + 
                  per65o + PerBachelorOrAbove + OOH_share + median_incomeLOG + districtwide + 
                  contest  + (1|distid), 
              data = moddat, REML = TRUE)

mixedMod2b <- lmer(turnout ~ CLOSE + VAP_adjLOG + CLOSE*VAP_adjLOG + per_white_all + 
                  per65o + PerBachelorOrAbove + OOH_share + median_incomeLOG + districtwide + 
                  contest + (1|year) + (1|distid), 
              data = moddat, REML = TRUE)
...
m0 <- lm(turnout ~  CLOSE + VAP_adjLOG + CLOSE*VAP_adjLOG + per_white_all + 
                  per65o + PerBachelorOrAbove + OOH_share + median_incomeLOG + districtwide + 
...
teachRLM <- ols(turnout ~ teachShareofVoters + fallTurnout + 
...
teachRLM2 <- ols(turnout ~ teachShareofVoters + fallTurnout + 
                    VAP_adjLOG + per_white_all +
                   per65o + PerBachelorOrAbove + OOH_share + median_incomeLOG + districtwide +
                  contest + year,
              data = moddat, x= TRUE, y = TRUE)
...
teachRLM3 <- ols(turnout ~ teachShareofVoters + fallTurnout + 
                    VAP_adjLOG + per_white_all +
                   per65o + PerBachelorOrAbove + OOH_share + median_incomeLOG + districtwide +
                  contest+ distid, 
              data = moddat, x = TRUE, y = TRUE)
...
mixedMod3 <- lmer(turnout ~ teachShareofVoters + fallTurnout + 
                    VAP_adjLOG + per_white_all +
                   per65o + PerBachelorOrAbove + OOH_share + median_incomeLOG + districtwide +
                  contest+ (1|distid), 
              data = moddat, REML = TRUE)

mixedMod3b <- lmer(turnout ~ teachShareofVoters + fallTurnout + 
                    VAP_adjLOG + per_white_all +
                   per65o + PerBachelorOrAbove + OOH_share + median_incomeLOG + districtwide +
                  contest+ (1|year) + (1|distid), 
              data = moddat, REML = TRUE)
...
m0 <- lm(turnout ~ teachShareofVoters + fallTurnout + 
                    VAP_adjLOG + per_white_all +
                   per65o + PerBachelorOrAbove + OOH_share + median_incomeLOG + districtwide +
                  contest, data = moddat) # fit the null model
...
m0 <- lm(turnoutDiff ~ turnoutLag2 * VAP_adjLOG + per_white_all + 
                   per65o + PerBachelorOrAbove + OOH_share + median_incomeLOG + CLOSE + 
                   contest + teachShareofVoters , data = moddat) # fit the null model
...
apsrtable(undvote1, caption = "Partial Pooling Model of School Board Undervote",
          model.names = c("Partial Pooled"),
...
apsrtable(mixedMod1b, mixedMod2b, mixedMod3b, mixedModCaus, 
          caption = "Partial Pooling Model of Post-Election Polarization",
          float = 'longtable', 
          model.names = c("Lagged", "Competitive", "Blank", "Causal"),
...
<<datasources, results='hide', eval=FALSE>>=

################################################################################
# Recall 2012
################################################################################

recall12 <- read.csv("Ward Election Data/2012/Spring 2012/JuneRecallFinal.csv")


recall12<-recall12[-grep("Totals", recall12$muni), ]

recall12 <- mcd_process(recall12)

# customizations

recall12$muniname3 <- gsub("COMBINED", "", recall12$muniname3)
recall12$muniname3 <- gsub("FIREHOUSE", "", recall12$muniname3)
recall12$muniname3 <- gsub("TOWNHALL", "", recall12$muniname3)
recall12$muniname3[recall12$muniname3 == "EDSONS"] <- "EDSON"
recall12$muniname3[recall12$muniname3 == "TRENTONS"] <- "TRENTON"
recall12$muniname3[recall12$muniname3 == "GLENDALEE"] <- "GLENDALE"
recall12$muniname3[recall12$muniname3 == "LAGRANGEAAA"] <- "LAGRANGE"
recall12$muniname3[recall12$muniname3 == "LAGRANGEBBB"] <- "LAGRANGE"
recall12$muniname3[recall12$muniname3 == "LOCKS"] <- "COMBINEDLOCKS"
recall12$muniname3[recall12$muniname3 == "BRILLIONS"] <- "BRILLION"



merge12<-merge(recall12,cw,by.x=c("muniname3","mcdType","county"),
                 by.y=c("name","mcdType","countyname"),all.x=TRUE)

length(merge12$dorCode[is.na(merge12$dorCode)])

#head(bigtest12.2[is.na(bigtest12.2$dorCode),],25)
merge12<-merge(merge12,sd12,by.x="dorCode",by.y="municode")
rm(recall12,sd12)

merge12$votes <- cleanvotes(merge12$votes)
merge12$rep <- cleanvotes(merge12$rep)
merge12$dem <- cleanvotes(merge12$dem)
merge12$ind <- cleanvotes(merge12$ind)
merge12$scatter <- cleanvotes(merge12$scatter)


distvotes12r<-ddply(merge12,.(distid),summarise,year="2012","RECTOT"=sum(votes*share,na.rm=T),
                   "GOVDEM"=sum(dem*share,na.rm=T), "GOVREP" = sum(rep * share, na.rm=T), 
                    "GOVIND" = sum(ind * share, na.rm=T), "GOVSCATT" = sum(scatter* share, na.rm=T))
rm(merge12)

save(distvotes12r, file = "../../../MasterText/data/cache/WalkerRecall.rda")
@