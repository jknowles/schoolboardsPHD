...
dumbRLM1 <- ols(turnout ~ turnoutLag2 + fallTurnout + log(VAP_adj) + per_white_all + 
...
dumbRLM1.2 <- ols(turnout ~ turnoutLag2 + fallTurnout + log(VAP_adj) + per_white_all + 
                    per65o + PerBachelorOrAbove + OOH_share + log(PCI) +
                    factor(year),
              data = moddat, x= TRUE, y = TRUE)
...
apsrtable(dumbRLM1, dumbRLM1.2, dumbRLM2, se="robust", label="tab:lagmods", 
...
# library(papeR)
# toLatex(prettify(summary(mixedMod1)))
