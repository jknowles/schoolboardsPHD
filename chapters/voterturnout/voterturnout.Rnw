...
\subsection{Under and Over Vote}
...
of undervote is variable across districts across years. Next, I explore whether 
...
mixedModCaus2 <- lmer(turnoutDiff*100 ~ turnoutLag2 + fallTurnout +  topTicketTurnoutSpring
                 VAP_adj + per_white_all + 
               per65o + PerBachelorOrAbove + OOH_share + PCI + CLOSE + 
               districtwide + contest + RecallPolarization*treatment + 
               treatment + turnout_overall_werc + (1|distid) + (1|year), 
