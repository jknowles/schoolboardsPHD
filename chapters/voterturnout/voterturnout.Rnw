...
plotdf <- merge(dist_turn, distAttr, by = c("distid", "year"))
plotdf$teachShareofVoters <- round(plotdf$fte_teachers,0) / plotdf$votersLag2
plotdf$teachShareofVoters[!is.finite(plotdf$teachShareofVoters)] <- 0

qplot(teachShareofVoters, turnout, 
      data = na.omit(plotdf[plotdf$year > 2004 & plotdf$turnout < 0.9 & plotdf$teachShareofVoters >0 ,])) + 
...
plot.tmp <- ddply(plotdf2, .(distid, year), summarize, 
                  races = length(distid), 
                  closeRaces = sum(closeRace))


plotdf <- merge(plotdf, plot.tmp, all.x=TRUE)
rm(plot.tmp, plotdf2, votes.tmp, cand.tmp)

plotdf$CLOSE <- factor(ifelse(plotdf$closeRaces >0, "Competitive", "Not Competitive"))

qplot(turnout, data = na.omit(plotdf[plotdf$turnout < 0.95,]), fill = CLOSE, 
      color = I("gray90")) +  
  theme_dpi() + 
...
\Sexpr{iNum(cor(plotdf$teachShareofVoters, plotdf$turnout, use = "pairwise.complete.obs"))}. 
