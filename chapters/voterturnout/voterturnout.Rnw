...
quite substantially. To combat this issue, I adopt the strategies outlined in 
Subsection \ref{subsec:spec} - year fixed effects, district fixed effects, and 
clustered standard errors. The results for these 
alternative specifications are displayed alongside the initial model in Table 
\ref{tab:lagmods}. 
...
apsrtable(closeRLM, closeRLM2, closeRLM3, se="robust", label="tab:closemods", 
...
<<model4, results='asis'>>=
mixedMod2 <- lmer(turnout ~ CLOSE + VAP_adj + per_white_all +
                   per65o + PerBachelorOrAbove + OOH_share + PCI + districtwide +
                    contest + (1|distid), 
              data = moddat, REML = TRUE)
