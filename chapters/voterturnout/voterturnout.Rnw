...
Prior turnout appears to be a substantively important predictor in Table \ref{tab:lagmods}.
\footnote{Using twice-lagged school board turnout. Results are not substantively different 
...
\ref{tab:closemod}. This provides evidence in support of the argument that school board 
...
apsrtable(teachRLM, teachRLM2, teachRLM3, se="robust", label="tab:teacmods", 
...
causal1 <- ols(turnout ~ turnoutLag2 + fallTurnout + VAP_adj + per_white_all + 
               ...
               turnout_overall_werc + treatment, 
              data = moddat, x = TRUE, y = TRUE)
