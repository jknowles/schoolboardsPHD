...
game amongst voters where the value of P is determined by the strategies selected by 
...
qplot(fallTwoPartyShareDem, turnout, data = na.omit(dist_turn[dist_turn$turnout < 0.95,])) + facet_wrap(~year) + 
  ...
  labs(x = "Democrat Share of Two Party Gubernatorial and Presidential Vote", y = "School Board Turnout", 
       title = "Comparing Turnout and Voter Partisanship in Wisconsin")
@

<<turnoutCompetitive, echo=FALSE, fig.cap="Turnout in School Board Elections based on Competitiveness">>=
...
qplot(turnout, data = na.omit(dist_turn[dist_turn$turnout < 0.95,]), fill = contest, 
  ...
  labs(x = "Turnout in School Board Elections", fill = "Type of Contestation", 
       y = "Count of Races",
       title = "Comparing Turnout and Race Typology in Wisconsin School Board Elections") + 
