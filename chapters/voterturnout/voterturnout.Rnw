...
qplot(turnoutLag2, turnout, data = na.omit(dist_turn[dist_turn$turnout < 0.9 & dist_turn$turnoutLag2 < 0.9,])) +
...
# 
# qplot(turnoutLag1, turnout, data = na.omit(dist_turn[dist_turn$turnout < 0.9 & dist_turn$turnoutLag1 < 0.9,])) +
#   facet_wrap(~year) + theme_dpi() + geom_smooth(method = 'rlm')+ 
#   scale_x_continuous(label = percent) + 
#   scale_y_continuous(label = percent) + 
#   labs(x = "Lagged School Board Turnout (1 year lag)", y = "School Board Turnout", 
#        title = "Comparing Lagged Turnout")

# 
# qplot(turnoutLag1, turnout, data = na.omit(dist_turn[dist_turn$turnout < 0.95,]), 
#       color = year) + 
#   theme_dpi() + geom_smooth(aes(group = year), method = 'lm')

#cor(dist_turn$turnout, dist_turn$turnoutLag1, use = "pairwise.complete.obs")
@
...
qplot(teachShareofVoters, turnout, 
  ...
  scale_x_continuous(label = percent) + 
  scale_y_continuous(label = percent) + 
  labs(x = "Teacher FTE as Share of School Board Voters", y = "School Board Turnout", 
...
# 
# qplot(turnout, data = dist_turn[dist_turn$turnout < 0.95 & !is.na(dist_turn$turnout),], fill = CLOSE, 
#       color = I("gray90")) +  
#   theme_dpi() + 
#   facet_wrap(~CLOSE, ncol = 1) +
#   scale_x_continuous(label = percent) + 
#   labs(x = "Turnout in School Board Elections", fill = "Competitiveness", 
#        y = "Count of Races",
...
qplot(minBlaisLago, turnout, data = dist_turn[dist_turn$turnout < 0.95 & !is.na(dist_turn$turnout) & dist_turn$minBlaisLago < 100 & 
  ...
  theme_dpi() + geom_smooth(method = 'rlm') +
  scale_y_continuous(label = percent) + 
  scale_x_sqrt() + 
  labs(x = "Turnout in School Board Elections", 
       y = "Blais-Lago Quotient",
       title = "Turnout and Competitiveness in School Board Elections") 
...
\Sexpr{iNum(cor(dist_turn$turnout, dist_turn$VAP_adj, use = "pairwise.complete.obs"))}. 
...
\Sexpr{iNum(cor(dist_turn$turnout, dist_turn$turnoutLag2, use = "pairwise.complete.obs"))}.
\footnote{For a single year lag, the corresponding correlation is 
\Sexpr{iNum(cor(dist_turn$turnout, dist_turn$turnoutLag1, use = "pairwise.complete.obs"))} and the 
...
\Sexpr{iNum(cor(dist_turn$turnout, dist_turn$fallTurnout, use = "pairwise.complete.obs"))}. 
...
\Sexpr{iNum(cor(dist_turn$turnout, abs(dist_turn$fallTwoPartyShareDem - .5), use = 'pairwise.complete.obs'))}. 
