...
mixedModcloseB <- lmer(turnout ~ minBlaisLago + VAP_adjLOG +  VAP_adjLOG_sq + 
                     fallTurnout +
                     minBlaisLago*VAP_adjLOG + per_white_all + 
                     per65o + PerBachelorOrAbove + OOH_share + 
...
mixedModcloseC <- update(mixedModcloseB, . ~ . - fallTurnout)
...
apsrtable(mixedModcloseB, mixedModcloseC, caption = "Partial Pooling Models of Competitiveness",
          model.names = c("Fall Turnout", "No Fall Turnout"),
...
% ##TODO: add interaction between teachShareofVoters and VAP_adjLOG, for coordination 
% # purposes it is easier for teachers to work together in smaller population. Turnout 
% # increases as teach share of voters increases, decreases as population increases
% # and decreases as size of district + teacher share increases due to coordination costs
...
apsrtable(mixedModteach, mixedModteachB, caption = "Partial Pooling Model of Staff Influence",
          model.names = c("Model 1","Model 2"),
            label = "tab:meInf", notes = list(tabnote, stars.note), 
...
partyCoefNames2 <- c("Intercept", "Fall Party Divide", "Fall Turnout", 
                 "Blais-Lago", "ln(VAP)", "\\% White", 
                  "\\% Over 65", "\\% Bach. +", "\\% OOH", 
                  "ln(MedianIncome)", "District-wide", "Open Seat Contested", 
                  "Uncontested Seat", "Party Division x VAP", "Fall 2 Party Dem.", 
                 "2 Party Dem x VAP")

mixedModP <- lmer(turnout ~ fallPartyDivision + fallTurnout + minBlaisLago + 
                    VAP_adjLOG + per_white_all + per65o + 
                   PerBachelorOrAbove + OOH_share + median_incomeLOG + 
                   districtwide + contest + fallPartyDivision * VAP_adjLOG + 
                    (1|distid) + (1|year), 
              data = moddat, REML = TRUE)
...
apsrtable(undervoteMMlag, undervoteMMclose,  undervoteMMcloseB,
          caption = "Partial Pooling Model of School Board Undervote",
           model.names = c("Lag", "Close Race", "Close Race 2"),
            label = "tab:meUndv1", notes = list(tabnote, stars.note), 
          Sweave = FALSE, float = "sidewaystable")
@
\end{tiny}
...
apsrtable(undervoteMMteach, undervoteMMparty, undervoteMMpartyB,
          caption = "Partial Pooling Model of School Board Undervote",
           model.names = c("Teacher Influence", "Party Influence", "Party Divide"),
            label = "tab:meUndv2", notes = list(tabnote, stars.note), 
          Sweave = FALSE)
