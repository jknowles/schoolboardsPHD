...
qplot(teachShareofVoters, turnout, 
      data = na.omit(plotdf[plotdf$teachShareofVoters < 0.4 & plotdf$year > 2004 & plotdf$turnout < 0.9 ,])) + 
...
<<exploratory, eval=FALSE, results='hide'>>=


qplot(topGovTurnoutPrior, turnout, data = dist_turn) + facet_wrap(~year) + 
  geom_smooth(se = TRUE, method = "lm") + theme_dpi()
qplot(topPresTurnoutPrior, turnout, data = dist_turn) + facet_wrap(~year)+ 
  geom_smooth(se = TRUE, method = "lm") + theme_dpi()



qplot(govTwoPartyShareDem -.5, turnout, data = na.omit(dist_turn[dist_turn$turnout < 0.85,])) + facet_wrap(~year) + 
  geom_smooth(se = TRUE) + theme_dpi()



qplot(presTwoPartyShareDem -.5, turnout, data = na.omit(dist_turn[dist_turn$turnout < 0.85,])) + facet_wrap(~year) + 
  geom_smooth(se = TRUE) + theme_dpi()
