...
dumbRLM1.2 <- ols(turnout ~ turnoutLag2 + fallTurnout + VAP_adjLOG + VAP_adjLOG_sq + 
                    per_white_all + 
                       per65o + PerBachelorOrAbove + OOH_share + median_incomeLOG + 
               districtwide +  minBlaisLago + 
                 factor(year), data = moddat, x= TRUE, y = TRUE)

dumbRLM1.2$se <- sqrt(diag(vcov(robcov(dumbRLM1.2, cluster = moddat$distid))))
...
apsrtable(dumbRLM1, dumbRLMbest, dumbRLM2, se="robust", 
          label="tab:lagmods", 
          model.names=c("Simple RLM",  "Interaction", 
                        "RLM + District FE"), 
          caption = "Models of Lagged Turnout in School Board Elections",
          notes = list("Standard errors clustered at the school district level.", 
                       stars.note, 
...
<<model1robustness>>=
# dumbRLM2 <- ols(turnout ~ turnoutLag2 + fallTurnout + log(VAP_adj) + per_white_all +
#                    per65o + PerBachelorOrAbove + OOH_share + log(median_income) +
#                   factor(distid), 
#               data = moddat, x = TRUE, y = TRUE)
# 
# pentrace(dumbRLM2, penalty = list(simple = 1:8))
# 
# dumbRLM2$se <- sqrt(diag(vcov(robcov(dumbRLM2, cluster = moddat$distid))))

naive2 <- lm(turnout ~ turnoutLag1 + fallTurnout + VAP_adjLOG + per_white_all + 
                       per65o + PerBachelorOrAbove + OOH_share + median_incomeLOG + 
               districtwide + 
               minBlaisLago,
             data = moddat)

turn21 <- ols(turnout ~ turnoutLag1 + fallTurnout + VAP_adjLOG + per_white_all + 
                       per65o + PerBachelorOrAbove + OOH_share + median_incomeLOG + 
               districtwide + 
               minBlaisLago, 
              data = moddat, x = TRUE, y = TRUE)

turn21$se <- sqrt(diag(vcov(robcov(turn21, cluster = moddat$distid))))


turn22 <- ols(turnout ~ turnoutLag1 + fallTurnout + VAP_adjLOG + per_white_all + 
                       per65o + PerBachelorOrAbove + OOH_share + median_incomeLOG + 
               districtwide + 
               minBlaisLago + factor(year),
              data = moddat, x= TRUE, y = TRUE)

turn22$se <- sqrt(diag(vcov(robcov(turn22, cluster = moddat$distid))))


turn23 <- ols(turnout ~ turnoutLag1 + fallTurnout + VAP_adjLOG + per_white_all + 
                       per65o + PerBachelorOrAbove + OOH_share + median_incomeLOG + 
               districtwide + 
               minBlaisLago + factor(distid), 
              data = moddat, x = TRUE, y = TRUE)

# pentrace(turn23, penalty = list(simple=c(0.25, 0.5, 0.75, 1, 2))) # unpenalized fit needed 
  # to trace and identify proper penalty value

turn23$se <- sqrt(diag(vcov(robcov(turn23, cluster = moddat$distid))))


appA1 <- apsrtable(naive2, turn21, turn22, turn23, se="robust", label="tab:lagmods2", 
          model.names=c("Naive OLS","Simple RLM", "RLM + Year FE", "RLM + District FE"), 
          caption = "Models of Lagged Turnout (1x) in School Board Elections",
          notes = list("Standard errors clustered at the school district.", 
                       stars.note, "Year and district fixed effects omitted."),
          omitcoef=list("distid", 
                        expression(grep("distid", coefnames, value=TRUE)), 
                        "year", 
                        expression(grep("year", coefnames, value = TRUE))), 
...
<<laggedTurnSens, results='hide', eval=FALSE>>=
library(RLRsim)
...
exactRLRT(m.slope, mixedMod1lagsb, m0)
...
apsrtable(closeRLM3, closeRLM4, se="robust", 
          label="tab:closemods", 
...
# library(MCMCpack)
# 
# fit1<-MCMCtobit(turnout ~ CLOSE + turnoutLag2 + fallTurnout + log(VAP_adj) + per_white_all +
#                    per65o + PerBachelorOrAbove + OOH_share + log(median_income),
#                 data=moddat,below=0,above=1,burnin=2000,mcmc=70000,thin=5)
# HPDinterval(fit1)
...
# summary(mixedMod2)
apsrtable(mixedModcloseB, caption = "Partial Pooling Model for Competitiveness",
...
<<closeModSens,results='hide', eval=FALSE>>=
...
exactRLRT(m.slope, mixedModcloseB, m0)
@
...
teachRLM3 <- ols(turnout ~ teachShareofVoters + fallTurnout + minBlaisLago + VAP_adjLOG_sq +
...
# Interaction model
teachRLM4 <- ols(turnout ~ teachShareofVoters + fallTurnout + minBlaisLago +  VAP_adjLOG_sq +
                    VAP_adjLOG + per_white_all + per65o + PerBachelorOrAbove +
                   OOH_share + median_incomeLOG + districtwide + contest + 
...
apsrtable(teachRLM3, teachRLM4, se="robust", label="tab:teachmods", 
...
mixedModteach <- lmer(turnout ~ teachShareofVoters +  minBlaisLago +  
                    VAP_adjLOG + per_white_all + per65o + PerBachelorOrAbove +
                   OOH_share + median_incomeLOG + districtwide + contest + 
                   teachShareofVoters * VAP_adjLOG + (1|distid), 
              data = moddat, REML = TRUE)

mixedModteachB <- lmer(turnout ~ teachShareofVoters + fallTurnout + minBlaisLago +  VAP_adjLOG_sq +
                    VAP_adjLOG + per_white_all + per65o + PerBachelorOrAbove +
                   OOH_share + median_incomeLOG + districtwide + contest + 
...
<<mixedInfmodSen, results='hide', eval=FALSE>>=
m0 <- lm(turnout ~ teachShareofVoters + fallTurnout + minBlaisLago + 
                    VAP_adjLOG + per_white_all + teachShareofVoters * VAP_adjLOG + 
                   per65o + PerBachelorOrAbove + OOH_share + median_incomeLOG +
                    districtwide + contest, data = moddat) # fit the null model
exactLRT(m = refitML(mixedModteach), m0 = m0)
# Test for year specific slopes
m.slope <- update(mixedModteachB, . ~ . - (1|year))
m0 <- update(mixedModteachB, . ~ . - (1|distid))
exactRLRT(m.slope, mixedModteachB, m0)
@
...
apsrtable(partyRLM1, partyRLM2, partyRLM3, partyRLM4, se="robust", label="tab:partymods", 
...
mixedModP <- lmer(turnout ~ fallPartyDivision*VAP_adjLOG + fallTurnout + minBlaisLago + 
                    VAP_adjLOG + per_white_all + per65o + 
                   PerBachelorOrAbove + OOH_share + median_incomeLOG + 
                   districtwide + contest + (1|distid) + (1|year), 
              data = moddat, REML = TRUE)

mixedModPb <- lmer(turnout ~ fallTwoPartyShareDem*VAP_adjLOG + fallTurnout + minBlaisLago + 
                    VAP_adjLOG + per_white_all + per65o + PerBachelorOrAbove +
                   OOH_share + median_incomeLOG + districtwide + contest + (1|year) + (1|distid), 
              data = moddat, REML = TRUE)
...
rm(dist.tmp, dat, cand, VAP_dist, distAttr, dvp, errors, govTurn, presTurn, races, 
   closeRLM, closeRLM2, closeRLM3, closeRLM4, dumbRLM1, dumbRLM1.2, dumbRLM2, 
   teachRLM, teachRLM2, teachRLM3, turn21, turn22, turn23, 
   naive1, naive2, mixedMod1, mixedMod2, mixedMod3, mixedModP, mixedModPb)
@
...
\begin{tiny}
<<undervoteMMfits, results='asis'>>=
mixedMod1lagsb <- lmer(turnoutDiff ~ turnoutLag2 + fallTurnout + 
...
mixedModclose <- lmer(turnoutDiff ~  minBlaisLago + VAP_adjLOG + 
                    minBlaisLago*VAP_adjLOG + per_white_all + 
                    per65o + PerBachelorOrAbove + OOH_share + 
                      ImputedFlag +
                    median_incomeLOG + districtwide + 
...
mixedModcloseB <- lmer(turnoutDiff ~ minBlaisLago + VAP_adjLOG +  VAP_adjLOG_sq + 
                     fallTurnout +
                     minBlaisLago*VAP_adjLOG + per_white_all + 
                     per65o + PerBachelorOrAbove + OOH_share + 
                       ImputedFlag +
...
mixedModteachB <- lmer(turnoutDiff ~ teachShareofVoters + fallTurnout + minBlaisLago +  VAP_adjLOG_sq +
                    VAP_adjLOG + per_white_all + per65o + PerBachelorOrAbove +
                      ImputedFlag +
                   OOH_share + median_incomeLOG + districtwide + contest + 
                   teachShareofVoters * VAP_adjLOG + (1|year) + (1|distid), 
...
mixedModPb <- lmer(turnoutDiff ~ fallTwoPartyShareDem*VAP_adjLOG + fallTurnout + minBlaisLago +
                     ImputedFlag + 
                    VAP_adjLOG + per_white_all + per65o + PerBachelorOrAbove +
                   OOH_share + median_incomeLOG + districtwide + contest + (1|year) + (1|distid), 
              data = moddat, REML = TRUE)
...
## Add teacher interaction here as well

# REtest1 <- exactLRT(m = refitML(undvote2), m0 = m0)
# 
# # Test for year specific slopes
# m.slope <- update(undvote1, . ~ . - (1|year))
# m0 <- update(undvote1, . ~ . - (1|distid))
# exactRLRT(m.slope, undvote1, m0)

apsrtable(mixedMod1lagsb, mixedModclose,  mixedModcloseB, mixedModteachB, mixedModPb,
          caption = "Partial Pooling Model of School Board Undervote",
#           model.names = c("Partial Pooled", "Teacher Share Interaction"),
            label = "tab:meUndv", notes = list(tabnote, stars.note), 
          Sweave = FALSE)
@
\end{tiny}
