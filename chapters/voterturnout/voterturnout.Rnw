...
Another social group to explore is political parties. Use measure of partisanship 
from average of two prior partisan general elections. 
...
partyCoefNames2 <- c("Intercept", "Fall Party Divide", "Fall Turnout", 
                 "Blais-Lago", "ln(VAP)", "\\% White", 
                  "\\% Over 65", "\\% Bach. +", "\\% OOH", 
                  "ln(MedianIncome)", "District-wide", "Party Division x VAP", "Fall 2 Party Dem.", 
                 "2 Party Dem x VAP")
...
% TODO: make RLM table for these models for appendix
...
undervoteMMclose <- lmer(turnoutDiff ~  minBlaisLago + VAP_adjLOG + 
              ...
              data = moddat, REML = TRUE)
undervoteMMcloseB <- lmer(turnoutDiff ~ minBlaisLago + VAP_adjLOG +  VAP_adjLOG_sq + 
                     fallTurnout +
                     minBlaisLago*VAP_adjLOG + per_white_all + 
                     per65o + PerBachelorOrAbove + OOH_share + 
                       ImputedFlag +
...
undervoteMMteach <- lmer(turnoutDiff ~ teachShareofVoters + fallTurnout + 
                  ...
                  OOH_share + median_incomeLOG + districtwide + 
                   ...
                   teachShareofVoters * VAP_adjLOG + (1|year) + (1|distid), 
              data = moddat, REML = TRUE)
undervoteMMparty <- lmer(turnoutDiff ~ fallTwoPartyShareDem*VAP_adjLOG + 
                  ...
                  VAP_adjLOG + per_white_all + per65o + PerBachelorOrAbove +
                   OOH_share + median_incomeLOG + districtwide + 
                    ImputedFlag + 
                     contest + (1|year) + (1|distid), 
              data = moddat, REML = TRUE)
undervoteMMpartyB <- lmer(turnoutDiff ~ fallPartyDivision*VAP_adjLOG + 
                           fallTurnout + minBlaisLago +
                  VAP_adjLOG + per_white_all + per65o + PerBachelorOrAbove +
                   OOH_share + median_incomeLOG + districtwide +
                    ImputedFlag +
...
coefNamesUnd1 <- c("Intercept", "Turnout Lag (2)","Fall Turnout", 
                 ...
                 "Blais-Lago", "Open Seat Contested", 
                  "Uncontested Seat", "Blais-Lago x Turnout")
...
coefNamesUnd2 <- c("Intercept", "Teacher Share VAP", "Fall Turnout", 
                  ...
                  "\\% Over 65", "\\% Bach. +", "\\% OOH", 
                  "ln(MedianIncome)", "District-wide", "Imputed",
                 "Open Seat Contested", "Uncontested Seat", 
...
Let's do some interpretation of the undervote. Table \ref{tab:meUndv1} explains 
undervote. 

Table \ref{tab:meUndv2} looks at influence and how it affects voter roll-off.
